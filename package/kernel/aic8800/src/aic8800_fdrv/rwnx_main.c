											   ...
											   struct vif_params *params)
{
	...
	if (type == NL80211_IFTYPE_AP_VLAN) {
		...
		i = rwnx_hw->wiphy->perm_addr[5];
		rwnx_hw->wiphy->perm_addr[5] ^= vif_idx;
		memcpy(ndev->dev_addr, rwnx_hw->wiphy->perm_addr, ETH_ALEN);
		rwnx_hw->wiphy->perm_addr[5] = i;
								 ...
								 struct cfg80211_csa_settings *params)
{
	...
	if (error) {
		...
		rwnx_cfg80211_ch_switch_started_notify(dev, &csa->chandef, 0, params->count, false, 0);
	}
