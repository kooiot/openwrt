...
#define DRIVER_DESC   "SPI serial driver for ch9434_c."
...
#define USE_SPI_MODE
#define CH943X_NAME_SPI "ch943x_spi_c"
...
static int ch943x_scr_test(struct uart_port *port)
{
	...
	if (val != 0x55) {
		dev_err(&s->spi_dev->dev, "UART %d SPR Test Failed. 0x55 got val: %x\n", port->line, val);
		return -1;
	}
	...
	val = ch943x_port_read(port, CH943X_SPR_REG);
	if (val != 0xAA) {
		dev_err(&s->spi_dev->dev, "UART %d SPR Test Failed. 0xAA got val: %x\n", port->line, val);
		return -1;
	}
...
static int ch943x_probe(struct spi_device *spi, struct ch943x_devtype *devtype, int irq, unsigned long flags)
{
	...
	for (i = 0; i < devtype->nr_uart; i++) {
		...
		if (spi->dev.of_node) {
			sprintf(buf, "port%d_rs485_mode", i);
			if (of_property_read_bool(spi->dev.of_node, buf)) {
				dev_info(dev, "enable RS485 mode\n");
				s->p[i].rs485.flags |= SER_RS485_ENABLED;
			} else {
				dev_info(dev, "disable RS485 mode\n");
				s->p[i].rs485.flags &= ~SER_RS485_ENABLED;
			}
		}
...
int ch9434_create_sysfs(struct spi_device *spi)
{
	...
	err = sysfs_create_link(NULL, &spi->dev.kobj, "ch9434_c");
	if (err < 0) {
