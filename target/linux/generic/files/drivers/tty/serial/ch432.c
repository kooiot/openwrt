...
static int ch43x_set_baud(struct uart_port *port, int baud)
{
    ...
    ch43x_port_update_specify(port, 1, CH43X_IER_REG, CH43X_IER_CK2X_BIT, prescaler);
...
static void ch43x_spi_test(struct uart_port *port)
{
    ...
    result = ch43x_port_read(port, CH43X_SPR_REG);

	if (result != 0x77) 
		printk("%s error, result:0x%02x\n", result);
