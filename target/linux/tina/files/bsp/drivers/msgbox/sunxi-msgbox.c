...
#define SUNXI_MSGBOX_VERSION()			(0x10)
...
int mbox_chan_get_msg_count(struct mbox_chan *chan)
{
	unsigned long flags;
	int msg_count;
					...
					void __iomem *base, int remote_n, int p)
{
	...
	while (msg_count) {
		fifo_msg_num = get_field(base + SUNXI_MSGBOX_MSG_STATUS(remote_n, p), MSG_NUM_MASK << MSG_NUM_SHIFT);
		if (fifo_msg_num >= chip->hwdata->fifo_msg_max) {
			spin_lock_irqsave(&chip->lock, flags);
			set_bits(base + SUNXI_MSGBOX_WRITE_IRQ_ENABLE(remote_n), WR_IRQ_EN_MASK<< WR_IRQ_EN_SHIFT(p));
			spin_unlock_irqrestore(&chip->lock, flags);
			break;
		}
		msg_count = mbox_chan_get_msg_count(chan);
...
static irqreturn_t sunxi_msgbox_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < chip->hwdata->mbox_num_chans; i++) {
		...
		if (read_irq_en && read_irq_pending && (irq == chip->irq[local_id])) {
			sunxi_msgbox_read_handler(chip, chan, read_reg_base, local_n, p);
			handled++;
		}

#if IS_ENABLED(CONFIG_AW_MAILBOX_SUPPORT_TXDONE_IRQ)
		if (write_irq_en && write_irq_pending && (irq == chip->irq[remote_id])) {
...
exit_none:
	dev_err(chip->dev, "nobody cares, irq no: %d\n", irq);
	dev_err(chip->dev, "irq_cnt=%d\n", chip->irq_cnt);
	for (i = 0; i < chip->irq_cnt; i++) {
		dev_err(chip->dev, "irq[%d]=%d\n", i, chip->irq[i]);
	}
	return IRQ_NONE;
...
static int msgbox_wait_writable(struct sunxi_msgbox *chip, int remote_id)
{
	...
	while (!msgbox_writable(write_reg_base)) {
		if (!wait_ms)
			dev_warn(chip->dev, " to processor %d: msgbox wait writable...\n", remote_id);
#ifdef MSGBOX_TX_WAIT_MS
		if (wait_ms >= MSGBOX_TX_WAIT_MS) {
			dev_err(chip->dev, " to processor %d: msgbox wait writable failed!\n", remote_id);
			return -1;
		}
#endif
		if (irqs_disabled())
	...
	if (wait_ms) {
		/*
		 * init specific config, if not default.
		 * write_irq_threshold is set to 1 (reg val is 0) when sunxi_msgbox_startup,
		 * this is the default val
		 */
		// sunxi_msgbox_set_write_irq_threshold(chip, write_reg_base, remote_n, p, 1);
		dev_warn(chip->dev, " to processor %d: msgbox writable now\n", remote_id);
	}
	return 0;
}
#endif
...
/* sun55iw6 core:
 *		core0:0,core1:1,core2:2,core3:3
 *    --------------------------------------------
 *         local_id        N        remote_id
 *     --------------------------------------------
 *             0           0            1
...
static int sunxi_msgbox_resource_get(struct sunxi_msgbox *chip)
{
	...
	chip->irq_cnt = chip->hwdata->processors_max;
