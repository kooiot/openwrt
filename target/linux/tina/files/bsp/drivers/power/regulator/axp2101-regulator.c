...
#define AXP_DESC_STEP(_family, _id, _match, _supply, _min, _max, _step, _vreg,	\
		 _vmask, _ereg, _emask) 					\
	[_family##_##_id] = {							\
		.name		= (_match),					\
		.supply_name	= (_supply),					\
...
int regulator_step_enable_regmap(struct regulator_dev *rdev)
{
	int val = 0;
	regmap_write(rdev->regmap, AXP2202_DCDC4_CFG, 0x0);

	regmap_read(rdev->regmap, AXP2202_DCDC4_CFG, &val);
...
static int regulator_is_enabled_regmap_axp515_drivevbus(struct regulator_dev *rdev)
{
	...
	ret = regmap_read(rdev->regmap, AXP515_MODULE_EN, &val[1]);
	if (ret)
		return ret;

	return ((val[0] &= BIT(6)) && (val[1] &= BIT(7)));
}
...
static int regulator_enable_regmap_axp515_drivevbus(struct regulator_dev *rdev)
{
	...
	ret = regmap_update_bits(rdev->regmap, AXP515_MODULE_EN, BIT(7), BIT(7));
	if (ret)
		return ret;
...
static int regulator_disable_regmap_axp515_drivevbus(struct regulator_dev *rdev)
{
	...
	ret = regmap_update_bits(rdev->regmap, AXP515_MODULE_EN, BIT(7), 0);
...
static struct regulator_ops axp20x_ops_step = {
	.set_voltage_sel	= regulator_set_voltage_sel_regmap,
	.get_voltage_sel	= regulator_get_voltage_sel_regmap,
	.list_voltage		= regulator_list_voltage_linear,
	.enable			= regulator_step_enable_regmap,
	.disable		= regulator_disable_regmap,
	.is_enabled		= regulator_is_enabled_regmap,
	.set_voltage_time_sel = axp2101_set_voltage_time_sel,
};
...
static const struct regulator_desc axp2202_regulators[] = {
	...
	AXP_DESC_STEP(AXP2202, DCDC4, "dcdc4", "vin-ps", 1000, 3700, 100,
