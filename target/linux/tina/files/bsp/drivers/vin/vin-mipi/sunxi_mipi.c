...
#if IS_ENABLED(CONFIG_ARCH_SUN8IW16P1)
	cmb_rx_set_base_addr(mipi->id, (vin_dma_addr_t)mipi->base);
#elif defined MIPI_COMBO_CSI
	cmb_csi_set_phy_base_addr(mipi->id, (vin_dma_addr_t)mipi->base);
	mipi->port_base = of_iomap(np, 1);
	if (!mipi->port_base) {
		ret = -EIO;
		goto freedev;
	}
	cmb_csi_set_port_base_addr(mipi->id, (vin_dma_addr_t)mipi->port_base);
#else
	bsp_mipi_csi_set_base_addr(mipi->id, (vin_dma_addr_t)mipi->base);
	bsp_mipi_dphy_set_base_addr(mipi->id, (vin_dma_addr_t)mipi->base + 0x1000);
#endif
