...
static void vin_md_set_power(struct vin_md *vind, int on)
{
	...
	if (on) {
		...
		csic_bk_intpool_int_src_sel(vind->id, FD_INT);
...
#if IS_ENABLED(CONFIG_ARCH_SUN50IW10)
			csic_ptn_control(vind->id, vinc->ptn_cfg.ptn_mode, vinc->ptn_cfg.ptn_dw, 2, vinc->ptn_cfg.ptn_gen_dly);
#else
			csic_ptn_control(vind->id, vinc->ptn_cfg.ptn_mode, vinc->ptn_cfg.ptn_dw, NCSIC3, vinc->ptn_cfg.ptn_gen_dly);
#endif
		else
#if IS_ENABLED(CONFIG_ARCH_SUN50IW10)
			csic_ptn_control(vind->id, vinc->ptn_cfg.ptn_mode, vinc->ptn_cfg.ptn_dw, 6, vinc->ptn_cfg.ptn_gen_dly);
#else
...
static int vin_create_media_links(struct vin_md *vind)
{
	...
	for (i = 0; i < VIN_MAX_DEV; i++) {
		...
		if (tdm_rx != NULL) {
			...
			sink = &isp->entity;
			ret = media_create_pad_link(source, TDM_PAD_SOURCE,
						sink, ISP_PAD_SINK,
						MEDIA_LNK_FL_ENABLED);
			vin_log(VIN_LOG_MD, "created link [%s] %c> [%s]\n",
				source->name, '=', sink->name);
