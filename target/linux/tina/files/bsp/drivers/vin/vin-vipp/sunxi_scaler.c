...
#endif
			vipp_set_osd_para_load_addr(scaler->id, (vin_dma_addr_t)scaler->osd_para.dma_addr);
			vipp_set_osd_stat_load_addr(scaler->id, (vin_dma_addr_t)scaler->osd_stat.dma_addr);
...
static int scaler_probe(struct platform_device *pdev)
{
	...
	vipp_set_base_addr(scaler->id, (vin_dma_addr_t)scaler->base);
	vipp_map_reg_load_addr(scaler->id, (vin_dma_addr_t)scaler->vipp_reg.vir_addr);
	vipp_map_osd_para_load_addr(scaler->id, (vin_dma_addr_t)scaler->osd_para.vir_addr);
