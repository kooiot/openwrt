...
#ifndef __CSIC__TDM230__REG__H__
#define __CSIC__TDM230__REG__H__

#include <uapi/media/sunxi_camera_v2.h>
#include <linux/types.h>
#include <media/v4l2-mediabus.h>
#include "tdm230_reg_i.h"
#include "../../platform/platform_cfg.h"

#define TDM_RX_NUM   4

#define MAX_TX_HEAD_FIFO_DEPTH		64
#define MAX_TX_DATA_FIFO_DEPTH		376
#define MAX_RX_HEAD_FIFO_DEPTH		128

#define MAX_RX_DATA_FIFO_DEPTH		624
#define RX0_DATA_FIFO_DEPTH		196
#define RX1_DATA_FIFO_DEPTH		196
#define RX2_DATA_FIFO_DEPTH		116
#define RX3_DATA_FIFO_DEPTH		116
...
enum tdm_int_sel {
	...
	AWNN_ID_BACK_ERR_INT_EN = 0X1 << 26,
	RX_LBC_MUX_CONF_TIME_OUT_INT_EN = 0X1 << 27,
	AWNN_ID_FRM_DONE_INT_EN = 0X1 << 28,
	RX_2TO1_FIFO_OV_INT_EN = 0X1 << 29,
	AWNN_TIME_OUT_BY_AWNN_INT_EN = 0X1 << 30,
	RX_W_ADDR_EXCEED_INT_EN = 0X1 << 31,
	TDM_INT_ALL = 0xFFFFFFFF,
};
...
enum tdm_tx_start_mode {
	AFTER_EX_FINISH = 0,
	AFTER_EX_N_LINE = 1,
};

enum tdm_rx_to_tx_mode {
	RX_TO_TX_ISP = 0,
	RX_TO_TX_SOFTWARE_CFG,
	RX_TO_TX_AI_ISP,
};
...
struct tdm_int_status {
	...
	bool awnn_id_back_err;
	bool rx_lbc_mux_conf_time_out;
	bool awnn_id_frm_done;
	bool rx_2to1_fifo_ov;
	bool awnn_time_out_by_awnn;
	bool rx_w_addr_exceed;
};
...
struct rx_aiisp_cfg0_t {
	bool rx_blc_en;
	bool rx_inv_blc_en;
	bool rx_normal_en;
	bool rx_inv_normal_en;
	bool rx_gamma_en;
	bool rx_inv_gamma_en;

	bool rx_tx_blc_en;
	bool rx_tx_inv_blc_en;
	bool rx_tx_normal_en;
	bool rx_tx_inv_normal_en;
};

struct rx_blc_offset {
	unsigned short r_offset;
	unsigned short g_offset;
	unsigned short b_offset;
};

struct rx_coef_offset {
	unsigned short r_coef;
	unsigned short g_coef;
	unsigned short b_coef;
};

struct rx_aiisp_cfg_t {
	unsigned int w_offset;
	unsigned int h_offset;
	unsigned int rx_line_stride1;
	unsigned int rx_hwc_fifo_addr;
	unsigned int rx_addr_offset1;
	unsigned int rx_buf_ov_rx_offset;
	unsigned int rx_buf_ov_tx_offset;
	struct rx_aiisp_cfg0_t rx_aiisp_cfg0;
	struct rx_blc_offset rx_blc_ofs;
};
...
#endif /* __CSIC__TDM230__REG__H__ */
