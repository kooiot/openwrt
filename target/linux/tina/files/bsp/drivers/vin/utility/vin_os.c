...
int vin_get_ion_phys(struct device *dev, struct vin_mm *mem_man)
{
	...
	sgt = dma_buf_map_attachment_unlocked(attachment, DMA_FROM_DEVICE);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	dma_buf_unmap_attachment_unlocked(attachment, sgt, DMA_FROM_DEVICE);
#else
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	dma_buf_unmap_attachment_unlocked(mem_man->attachment, mem_man->sgt, DMA_FROM_DEVICE);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	ret = dma_buf_vmap_unlocked(mem_man->buf, &map);
	if (ret) {
...
err_phys:
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	dma_buf_vunmap_unlocked(mem_man->buf, mem_man->vir_addr);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	if (mem_man->vir_addr)
		dma_buf_vunmap_unlocked(mem_man->buf, &map);
	dma_heap_buffer_free(mem_man->buf);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	if (mem_man->vir_addr)
