...
static int csi_probe(struct platform_device *pdev)
{
	...
	ret = csic_prs_set_base_addr(csi->id, (vin_dma_addr_t)csi->base);
	if (ret < 0)
		goto unmap;
...
int sunxi_csi_set_ch_fps_mode(struct v4l2_subdev *sd)
{
	struct csi_dev *csi = v4l2_get_subdevdata(sd);
	struct vin_md *vind = dev_get_drvdata(csi->subdev.v4l2_dev->dev);
	int i;
	...
	csi->prs_fps_ds.ch0_fps_ds = 1;
	csi->prs_fps_ds.ch1_fps_ds = 1;
	csi->prs_fps_ds.ch0_frm_data_mask = 0x1;//01b
	csi->prs_fps_ds.ch1_frm_data_mask = 0x2;//10b
