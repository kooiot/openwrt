...
#include "dwmac-ethercat-sunxi-sysfs.h"
...
static int sunxi_ethercat_dwmac_init(struct platform_device *pdev, void *priv)
{
...
static void sunxi_ethercat_dwmac_exit(struct platform_device *pdev, void *priv)
{
	struct sunxi_dwmac *chip = priv;
...
#endif
static int sunxi_ethercat_dwmac_probe(struct platform_device *pdev)
{
	struct plat_stmmacenet_data *plat_dat;
	...
	plat_dat->init = sunxi_ethercat_dwmac_init;
	plat_dat->exit = sunxi_ethercat_dwmac_exit;
	/* must use 0~4G space */
	...
	ret = sunxi_ethercat_dwmac_init(pdev, plat_dat->bsp_priv);
	...
	ret =  stmmac_ethercat_dvr_probe(&pdev->dev, plat_dat, &stmmac_res);
	...
	sunxi_dwmac_ethercat_sysfs_init(&pdev->dev);
	
	//printk("=====YTR====probe success (Version %s)\n", DWMAC_MODULE_VERSION);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
err_cfg:
	stmmac_ethercat_dvr_remove(&pdev->dev);
#endif
err_dvr_probe:
	sunxi_ethercat_dwmac_exit(pdev, chip);
err_init:
	stmmac_remove_config_dt(pdev, plat_dat);
	return ret;
}

static int sunxi_ethercat_dwmac_remove(struct platform_device *pdev)
{
	sunxi_dwmac_ethercat_sysfs_exit(&pdev->dev);
	stmmac_pltfr_remove(pdev);
	return 0;
}

static void sunxi_ethercat_dwmac_shutdown(struct platform_device *pdev)
{
...
static int __maybe_unused sunxi_dwmac_suspend(struct device *dev)
{
	...
	ret = stmmac_ethercat_suspend(dev);
	sunxi_ethercat_dwmac_exit(pdev, chip);
	stmmac_ethercat_bus_clks_config(priv, false);
...
static int __maybe_unused sunxi_dwmac_resume(struct device *dev)
{
	...
	stmmac_ethercat_bus_clks_config(priv, true);
	sunxi_ethercat_dwmac_init(pdev, chip);
	if (ndev && ndev->phydev) {
		phy_device_reset(ndev->phydev, 1);
		phy_device_reset(ndev->phydev, 0);
	}
	ret = stmmac_ethercat_resume(dev);
...
static const struct sunxi_dwmac_variant dwmac200_ethercat_variant = {
...
static const struct sunxi_dwmac_variant dwmac210_ethercat_variant = {
	.interface = PHY_INTERFACE_MODE_RMII | PHY_INTERFACE_MODE_RGMII,
...
static const struct sunxi_dwmac_variant dwmac220_ethercat_variant = {
	.interface = PHY_INTERFACE_MODE_RMII | PHY_INTERFACE_MODE_RGMII,
	//.flags = SUNXI_DWMAC_SPH_DISABLE | SUNXI_DWMAC_NSI_CLK_GATE | SUNXI_DWMAC_MULTI_MSI | SUNXI_DWMAC_MEM_ECC,
	.flags = SUNXI_DWMAC_SPH_DISABLE | SUNXI_DWMAC_NSI_CLK_GATE | SUNXI_DWMAC_MULTI_MSI,
	.rx_delay_max = 31,
	.tx_delay_max = 31,
	.set_syscon = sunxi_dwmac200_set_syscon,
	.set_delaychain = sunxi_dwmac210_set_delaychain,
...
static const struct of_device_id sunxi_ethercat_dwmac_match[] = {
	 { .compatible = "allwinner,sunxi-gmac-ethercat-200", .data = &dwmac200_ethercat_variant },
	//{ .compatible = "allwinner,sunxi-gmac-ethercat-210", .data = &dwmac210_variant },
	 { .compatible = "allwinner,sunxi-gmac-ethercat-220", .data = &dwmac220_ethercat_variant },
	//{ .compatible = "allwinner,sunxi-gmac-ethercat-110", .data = &dwmac110_variant },
	{ }
};
MODULE_DEVICE_TABLE(of, sunxi_ethercat_dwmac_match);

static struct platform_driver sunxi_ethercat_dwmac_driver = {
	.probe = sunxi_ethercat_dwmac_probe,
	.remove = sunxi_ethercat_dwmac_remove,
	.shutdown = sunxi_ethercat_dwmac_shutdown,
	.driver = {
		.name			= "dwmac-sunxi-ethercat",
		.pm		= &sunxi_dwmac_pm_ops,
		.of_match_table = sunxi_ethercat_dwmac_match,
	},
};
module_platform_driver(sunxi_ethercat_dwmac_driver);
