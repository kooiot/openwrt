...
#include "ccu-sun8iw22-rtc.h"

#define SUNXI_RTC_CCU_VERSION       "0.0.2"
...
/* rtc_spi_clk_gate */
static SUNXI_CCU_GATE(rtc_spi_clk, "rtc-spi", "ahbs-clk", 0x310, BIT(31), 0);

static struct ccu_common *sun8iw22_rtc_ccu_clks[] = {
...
static struct clk_hw_onecell_data sun8iw22_rtc_ccu_hw_clks = {
	.hws	= {
...
static const struct sunxi_ccu_desc sun8iw22_rtc_ccu_desc = {
	.ccu_clks	= sun8iw22_rtc_ccu_clks,
	.num_ccu_clks	= ARRAY_SIZE(sun8iw22_rtc_ccu_clks),

	.hw_clks	= &sun8iw22_rtc_ccu_hw_clks,
};

static void clock_source_init(char __iomem *base)
{
	u32 val = 0;
	u32 ext_losc_sta_shift = 2;
	u32 ext_losc_sta_mask = 0x1;
	u32 read_times = 0;
	u32 normal_times = 0;

	/* select the LOSC_SRC(rc16m) as rtc clock source, and use auto switch */
	set_reg_key(base + LOSC_CTRL_REG,
		    (KEY_FIELD_MAGIC_NUM_RTC >> 16), 16, 16,
		    0x0, 1, 1);
	/* step1 */
	set_reg(base + LOSC_CTRL_REG, 0x1, 1, 14);
	/* step2 */
	set_reg(base + LOSC_CTRL_REG, 0x1, 1, 4);
	/* step3 */
	set_reg(base + LOSC_CTRL_REG, 0x0, 1, 15);
	/* step4 */
	while ((read_times != 100) && (normal_times != 4)) {
		val = readl(base + LOSC_AUTO_SWT_STA_REG);
		val = ((val >> ext_losc_sta_shift) & ext_losc_sta_mask);
		if (!val)
			normal_times += 1;
		else
			normal_times = 0;
		...
		if (normal_times < 4)
			pr_err("losc not stable, 32K clk will use 16M as parent!\n");
	};
	/* step5 */
	set_reg_key(base + LOSC_CTRL_REG,
		    (KEY_FIELD_MAGIC_NUM_RTC >> 16), 16, 16,
...
static int sun8iw22_rtc_ccu_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	return sunxi_ccu_probe(pdev->dev.of_node, reg, &sun8iw22_rtc_ccu_desc);
}

static const struct of_device_id sun8iw22_rtc_ccu_ids[] = {
	{ .compatible = "allwinner,sun8iw22-rtc-ccu" },
	{ }
};

static struct platform_driver sun8iw22_rtc_ccu_driver = {
	.probe	= sun8iw22_rtc_ccu_probe,
	.driver	= {
		.name	= "sun8iw22-rtc-ccu",
		.of_match_table	= sun8iw22_rtc_ccu_ids,
	},
};

static int __init sun8iw22_rtc_ccu_init(void)
{
	int err;

	err = platform_driver_register(&sun8iw22_rtc_ccu_driver);
	if (err)
		pr_err("Fail to register sunxi_rtc_ccu as platform device\n");

	return err;
}
core_initcall(sun8iw22_rtc_ccu_init);

static void __exit sun8iw22_rtc_ccu_exit(void)
{
	platform_driver_unregister(&sun8iw22_rtc_ccu_driver);
}
module_exit(sun8iw22_rtc_ccu_exit);

MODULE_DESCRIPTION("sunxi RTC CCU driver");
MODULE_AUTHOR("<haili@allwinnertech.com>");
