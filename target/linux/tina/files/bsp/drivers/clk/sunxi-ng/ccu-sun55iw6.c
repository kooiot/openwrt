...
#define SUNXI_CCU_VERSION		"0.6.3"
...
#define SUN55IW6_PLL_AUDIO0_CTRL_REG		0x260
static struct ccu_sdm_setting pll_audio0_sdm_table[] = {
	{ .rate = 45158400, .pattern = 0xc00028f5, .m = 50, .n = 94 }, /* 22.5792 */
};
...
#define SUN55IW6_PLL_AUDIO1_CTRL_REG		0x280
...
static struct ccu_nm pll_audio1_4x_clk = {
	...
	.common		= {
		.reg		= 0x280,
		.features	= CCU_FEATURE_SIGMA_DELTA_MOD,
		.hw.init	= CLK_HW_INIT("pll-audio1-4x", "dcxo24M",
				&ccu_nm_ops,
				CLK_SET_RATE_UNGATE),
	},
};
...
static SUNXI_CCU_M_WITH_MUX(apb1_clk, "apb1", apb1_parents,
		0x0518, 0, 5, 24, 2, CLK_SET_RATE_PARENT | CLK_IS_CRITICAL);
...
static SUNXI_CCU_M_WITH_MUX(apb_uart_clk, "apb-uart", apb_uart_parents,
		0x0538, 0, 5, 24, 2, CLK_IS_CRITICAL);
...
static const char * const mbus_parents[] = { "dcxo24M", "pll-peri0-600m", "pll-peri0-480m", "pll-peri0-400m", "pll-ddr", "pll-npu-4x" };
...
#define AHB_KEY_VALUE	0xE
static SUNXI_CCU_GATE_WITH_KEY(vid_out_ahb_gate_clk, "vid-out-gate",
		"dcxo24M", 0x05C0,
		AHB_KEY_VALUE,
		BIT(3), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE_WITH_KEY(vid_in_ahb_gate_clk, "vid-in-gate",
		"dcxo24M", 0x05C0,
		AHB_KEY_VALUE,
		BIT(2), CLK_IS_CRITICAL);

#define MBUS_KEY_VALUE    0x302
...
static SUNXI_CCU_GATE_WITH_KEY(csi_mbus_gate_clk, "csi-mbus-gate",
		"dcxo24M", 0x05E0,
		MBUS_KEY_VALUE,
		BIT(8), CLK_IS_CRITICAL);

#define MBUS_MASTER_KEY_VALUE	0x920000
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(nand_mbus_gate_clk, "nand-mbus-gate",
		"dcxo24M",
		0x05E0, MBUS_KEY_VALUE, BIT(5),
		0x05E4, MBUS_MASTER_KEY_VALUE, BIT(22),
		CLK_IGNORE_UNUSED);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(dma1_mbus_gate_clk, "dma1-mbus-gate",
		"dcxo24M",
		0x05E0, MBUS_KEY_VALUE, BIT(3),
		0x05E4, MBUS_MASTER_KEY_VALUE, BIT(29),
		CLK_IGNORE_UNUSED);

static SUNXI_CCU_GATE_ASSOC_WITH_KEY(ce_mbus_gate_clk, "ce-mbus-gate",
		"dcxo24M",
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(ve_mbus_gate_clk, "ve-mbus-gate",
		"dcxo24M",
		0x05E0, MBUS_KEY_VALUE, BIT(1),
		0x05E4, MBUS_MASTER_KEY_VALUE, BIT(17),
		CLK_IS_CRITICAL);

static SUNXI_CCU_GATE_ASSOC_WITH_KEY(dma0_mbus_gate_clk, "dma0-mbus-gate",
		"dcxo24M",
		0x05E0, MBUS_KEY_VALUE, BIT(0),
		0x05E4, MBUS_MASTER_KEY_VALUE, BIT(28),
		CLK_IGNORE_UNUSED);

static SUNXI_CCU_GATE_WITH_KEY(vo_sys_mbus_gate_clk, "vo-sys-mbus-gate",
		"dcxo24M", 0x05E4,
		MBUS_MASTER_KEY_VALUE,
		BIT(23), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE_WITH_KEY(vid_in_mbus_gate_clk, "vid-sys-mbus-gate",
		"dcxo24M", 0x05E4,
		MBUS_MASTER_KEY_VALUE,
		BIT(20), CLK_IS_CRITICAL);
...
static struct ccu_div timer0_clk = {
	...
	.common		= {
		.reg		= 0x0800,
		.hw.init	= CLK_HW_INIT_PARENTS("timer0-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer1_clk = {
	...
	.common		= {
		.reg		= 0x0804,
		.hw.init	= CLK_HW_INIT_PARENTS("timer1-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer2_clk = {
	...
	.common		= {
		.reg		= 0x0808,
		.hw.init	= CLK_HW_INIT_PARENTS("timer2-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer3_clk = {
	...
	.common		= {
		.reg		= 0x080C,
		.hw.init	= CLK_HW_INIT_PARENTS("timer3-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer4_clk = {
	...
	.common		= {
		.reg		= 0x0810,
		.hw.init	= CLK_HW_INIT_PARENTS("timer4-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer5_clk = {
	...
	.common		= {
		.reg		= 0x0814,
		.hw.init	= CLK_HW_INIT_PARENTS("timer5-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer6_clk = {
	...
	.common		= {
		.reg		= 0x0818,
		.hw.init	= CLK_HW_INIT_PARENTS("timer6-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer7_clk = {
	...
	.common		= {
		.reg		= 0x081C,
		.hw.init	= CLK_HW_INIT_PARENTS("timer7-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static struct ccu_div timer0_rv_clk = {
	.enable		= BIT(31),
	.div		= _SUNXI_CCU_DIV_FLAGS(0, 3, CLK_DIVIDER_POWER_OF_TWO),
	.mux		= _SUNXI_CCU_MUX(24, 3),
	.common		= {
		.reg		= 0x0860,
		.hw.init	= CLK_HW_INIT_PARENTS("timer0-rv-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};

static struct ccu_div timer1_rv_clk = {
	.enable		= BIT(31),
	.div		= _SUNXI_CCU_DIV_FLAGS(0, 3, CLK_DIVIDER_POWER_OF_TWO),
	.mux		= _SUNXI_CCU_MUX(24, 3),
	.common		= {
		.reg		= 0x0864,
		.hw.init	= CLK_HW_INIT_PARENTS("timer1-rv-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};

static struct ccu_div timer2_rv_clk = {
	.enable		= BIT(31),
	.div		= _SUNXI_CCU_DIV_FLAGS(0, 3, CLK_DIVIDER_POWER_OF_TWO),
	.mux		= _SUNXI_CCU_MUX(24, 3),
	.common		= {
		.reg		= 0x0868,
		.hw.init	= CLK_HW_INIT_PARENTS("timer2-rv-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};

static struct ccu_div timer3_rv_clk = {
	.enable		= BIT(31),
	.div		= _SUNXI_CCU_DIV_FLAGS(0, 3, CLK_DIVIDER_POWER_OF_TWO),
	.mux		= _SUNXI_CCU_MUX(24, 3),
	.common		= {
		.reg		= 0x086C,
		.hw.init	= CLK_HW_INIT_PARENTS("timer3-rv-clk",
				timer0_clk_parents,
				&ccu_div_ops, 0),
	},
};
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(ve_bus_clk, "ve-bus",
		"dcxo24M",
		0x0A8C, 0, BIT(0),
...
static SUNXI_CCU_MP_MIN_WITH_MUX_GATE_NO_INDEX(npu_clk, "npu",
		npu_parents, 0x0B00,
		0, 5, 1, /* M m_min */
		0, 0, /* P */
		24, 3, /* mux */
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(npu_bus_clk, "npu-bus",
		"dcxo24M",
		0x0B04, 0, BIT(0),
		0x05C0, AHB_KEY_VALUE, BIT(15),
		CLK_IGNORE_UNUSED);
...
static SUNXI_CCU_M(e907_axi_clk, "e907-axi",
		"rv-core", 0xB80, 8, 2, 0);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(smhc0_bus_clk, "smhc0-bus",
		"dcxo24M",
		0x0D0C, 0, BIT(0),
		0x05C0, AHB_KEY_VALUE, BIT(17) | BIT(5),
		CLK_IGNORE_UNUSED);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(smhc1_bus_clk, "smhc1-bus",
		"dcxo24M",
		0x0D1C, 0, BIT(0),
		0x05C0, AHB_KEY_VALUE, BIT(18) | BIT(6),
		CLK_IGNORE_UNUSED);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(smhc2_bus_clk, "smhc2-bus",
		"dcxo24M",
		0x0D2C, 0, BIT(0),
		0x05C0, AHB_KEY_VALUE, BIT(19) | BIT(7),
		CLK_IGNORE_UNUSED);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(spif_bus_clk, "spif-bus",
		"dcxo24M",
		0x0F1C, 0, BIT(0),
		0x05C0, AHB_KEY_VALUE, BIT(22),
		CLK_IGNORE_UNUSED);
...
static const char * const can0_parents[] = { "pll-peri0-400m" };

static SUNXI_CCU_M_WITH_MUX_GATE(can0_clk, "can0",
		can0_parents, 0x0F80,
		0, 5,	/* M */
		24, 1,	/* mux not used */
		BIT(31),	/* gate */
...
static const char * const can1_parents[] = { "pll-peri0-400m" };

static SUNXI_CCU_M_WITH_MUX_GATE(can1_clk, "can1",
		can1_parents, 0x0F88,
		0, 5,	/* M */
		24, 1,	/* mux not used */
		BIT(31),	/* gate */
...
static const char * const can2_parents[] = { "pll-peri0-400m" };

static SUNXI_CCU_M_WITH_MUX_GATE(can2_clk, "can2",
		can2_parents, 0x0F90,
		0, 5,	/* M */
		24, 1,	/* mux not used */
		BIT(31),	/* gate */
...
static SUNXI_CCU_GATE(can2_bus_clk, "can2-bus",
		"dcxo24M",
		0x0F94, BIT(0), 0);

static const char * const can3_parents[] = { "pll-peri0-400m" };

static SUNXI_CCU_M_WITH_MUX_GATE(can3_clk, "can3",
		can3_parents, 0x0F98,
		0, 5,	/* M */
		24, 1,	/* mux not used */
		BIT(31),	/* gate */
...
static SUNXI_CCU_GATE(can3_bus_clk, "can3-bus",
		"dcxo24M",
		0x0F9c, BIT(0), 0);
...
static const char * const tpadc_24m_parents[] = { "dcxo24M", "pll-audio0-4x" };
...
static const char * const lbc_parents[] = { "pll-peri0-480m", "pll-peri0-400m", "pll-peri0-300m", "pll-peri1-480m", "pll-peri1-400m", "pll-peri1-300m", "pll-video0-1x", "pll-video1-3x" };
...
static const char * const i2spcm0_parents[] = { "pll-audio0-4x", "pll-audio1-4x", "pll-peri0-200m" };
...
static const char * const owa_tx_parents[] = { "pll-audio0-4x", "pll-audio1-4x" };
...
static const char * const owa_rx_parents[] = { "pll-peri0-400m", "pll-peri0-300m", "pll-audio0-4x", "pll-audio1-4x" };
...
static const char * const audio_codec_dac_1x_parents[] = { "pll-audio0-4x", "pll-audio1-4x" };
...
static SUNXI_CCU_GATE(usb0_usb1_24m_clk, "usb0-usb1-24m",
		"dcxo24M",
		0x1310, BIT(0), 0);
...
static SUNXI_CCU_GATE(serdes_bus_clk, "serdes-bus",
		"dcxo24M",
		0x13CC, BIT(1), 0);

static SUNXI_CCU_GATE(serdes_ahb_clk, "serdes-ahb",
		"dcxo24M",
		0x13CC, BIT(0), 0);

static const char * const gmac0_phy_parents[] = { "pll-peri0-150m" };
static SUNXI_CCU_M_WITH_MUX_GATE(gmac0_phy_clk, "gmac0-phy",
		gmac0_phy_parents, 0x1400,
		0, 5,	/* M */
		24, 2,	/* mux: not used */
		BIT(31),	/* gate */
		0);
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(gmac0_clk, "gmac0",
		"dcxo24M",
		0x140C, 0, BIT(0),
...
static SUNXI_CCU_GATE_ASSOC_WITH_KEY(gmac1_clk, "gmac1",
		"dcxo24M",
		0x141C, 0, BIT(0),
...
static const char * const vo0_combphy0_parents[] = { "pll-video0-4x", "pll-video1-4x", "pll-video0-1x", "pll-video1-3x", "pll-peri0-2x" };
...
static const char * const csi_master0_parents[] = { "dcxo24M", "pll-video1-4x", "pll-video1-3x", "pll-video0-4x", "pll-video0-1x" };
...
static const char * const csi_master1_parents[] = { "dcxo24M", "pll-video1-4x", "pll-video1-3x", "pll-video0-4x", "pll-video0-1x" };
...
static const char * const csi_master2_parents[] = { "dcxo24M", "pll-video1-4x", "pll-video1-3x", "pll-video0-4x", "pll-video0-1x" };
...
static const char * const csi_master3_parents[] = { "dcxo24M", "pll-video1-4x", "pll-video1-3x", "pll-video0-4x", "pll-video0-1x" };
...
static const char * const csi_parents[] = { "pll-peri0-480m", "pll-peri0-400m", "pll-peri0-300m", "pll-video0-4x", "pll-video0-1x", "pll-video1-4x", "pll-video1-3x", "pll-ve" };
...
static const char * const isp_parents[] = { "pll-peri0-300m", "pll-peri0-400m", "pll-video0-4x", "pll-video0-1x", "pll-video1-4x", "pll-video1-3x", "pll-ve", "pll-npu-4x" };
...
static struct ccu_nm pll_audio0_sdm_clk = {
	.common		= {
		.reg		= 0x268,
	},
};
static struct ccu_nm pll_audio1_sdm_clk = {
	.common		= {
		.reg		= 0x288,
	},
};
...
static struct ccu_reset_map sun55iw6_ccu_resets[] = {
	...
	[RST_BUS_NPU_TZMA_AHB]		= { 0x0b04, BIT(21) },
	[RST_BUS_NPU_TZMA_APB]		= { 0x0b04, BIT(20) },
	...
	[RST_BUS_CAN2]			= { 0x0f94, BIT(16) },
	[RST_BUS_CAN3]			= { 0x0f9c, BIT(16) },
...
static struct clk_hw_onecell_data sun55iw6_hw_clks = {
	.hws    = {
		...
		[CLK_AHB_VID_OUT_GATE]		= &vid_out_ahb_gate_clk.common.hw,
		[CLK_AHB_VID_IN_GATE]		= &vid_in_ahb_gate_clk.common.hw,
		[CLK_MBUS_CAN_GATE]		= &can_mbus_gate_clk.common.hw,
		[CLK_MBUS_GMAC1_GATE]		= &gmac1_mbus_gate_clk.common.hw,
		[CLK_MBUS_GMAC0_GATE]		= &gmac0_mbus_gate_clk.common.hw,
		[CLK_MBUS_ISP_GATE]		= &isp_mbus_gate_clk.common.hw,
		[CLK_MBUS_CSI_GATE]		= &csi_mbus_gate_clk.common.hw,
		[CLK_MBUS_NAND_GATE]		= &nand_mbus_gate_clk.common.hw,
		[CLK_MBUS_DMA1_GATE]		= &dma1_mbus_gate_clk.common.hw,
		[CLK_MBUS_CE_GATE]		= &ce_mbus_gate_clk.common.hw,
		[CLK_MBUS_VE_GATE]		= &ve_mbus_gate_clk.common.hw,
		[CLK_MBUS_DMA0_GATE]		= &dma0_mbus_gate_clk.common.hw,
		[CLK_MBUS_VO_SYS_GATE]		= &vo_sys_mbus_gate_clk.common.hw,
		...
		[CLK_BUS_TIMER_RV]		= &timer_rv_bus_clk.common.hw,
		...
		[CLK_E907_AXI]			= &e907_axi_clk.common.hw,
		...
		[CLK_CAN2]			= &can2_clk.common.hw,
		[CLK_BUS_CAN2]			= &can2_bus_clk.common.hw,
		[CLK_CAN3]			= &can3_clk.common.hw,
		[CLK_BUS_CAN3]			= &can3_bus_clk.common.hw,
		...
		[CLK_USB0_USB1_24M]		= &usb0_usb1_24m_clk.common.hw,
		...
		[CLK_BUS_SERDES]		= &serdes_bus_clk.common.hw,
		[CLK_AHB_SERDES]		= &serdes_ahb_clk.common.hw,
...
static struct ccu_common *sun55iw6_ccu_clks[] = {
	...
	&serdes_bus_clk.common,
	&serdes_ahb_clk.common,
	...
	&isp_bus_clk.common,
#ifdef CONFIG_PM_SLEEP
	&pll_audio0_sdm_clk.common,
	&pll_audio1_sdm_clk.common,
#endif
};

static struct ccu_reg_dump sun55iw6_distbus_restore_clks[] = {
	{ 0x05C0, 0xE }, /* AHB_GATE_EN_REG */
	{ 0x05E0, 0x302 }, /* MBUS_GATE_EN_REG */
	{ 0x05E4, 0x920000 }, /* MBUS_MAT_CLK_GATING_REG */
...
static void sun55iw6_clk_update_npu(void __iomem *reg, u32 npu_key)
{
	switch (npu_key) {
	case NPU_LEVEL_0:
		sunxi_info(NULL, "sun55iw6 use default npu clk config!\n");
		return;
	case NPU_LEVEL_1:
		sunxi_info(NULL, "sun55iw6 not use npu!\n");
		return;
	case NPU_LEVEL_2:
		npu_clk.m.min = 6;
		break;
	...
	case NPU_LEVEL_5:
		...
		break;
	default:
		sunxi_err(NULL, "sun55iw6 get npu config err!\n");
		return;
	}

	pll_npu_4x_clk.n.min = 105;
	pll_npu_4x_clk.n.max = 164;

	pll_npu_4x_clk.min_rate = 2520000000;
	pll_npu_4x_clk.max_rate = 3936000000;

	pll_npu_4x_clk.m.shift = 0;
	pll_npu_4x_clk.m.width = 0;
	pll_npu_4x_clk.m.max = 0;
	pll_npu_4x_clk.m.min = 0;
	pll_npu_4x_clk.m.offset = 0;
	pll_npu_4x_clk.m.flags = 0;
	pll_npu_4x_clk.m.table = 0;

	pll_npu_4x_clk.fixed_post_div = 2,
	pll_npu_4x_clk.common.features = CCU_FEATURE_FIXED_POSTDIV;

	set_reg(reg + SUN55IW6_PLL_NPU_CTRL_REG, 0x0, 3, 20);

	sunxi_debug(NULL, "use fixed npu factor m: %d\n", npu_clk.m.min);
}

#define SUN55IW6_PLL_PERI0_EN_REG 	0x1908
#define SUN55IW6_PLL_PERI1_EN_REG 	0x190C
#define SUN55IW6_PLL_VIDEO_EN_REG 	0x1910
static int sun55iw6_ccu_really_probe(struct device_node *node)
{
	void __iomem *reg;
	u32 ret;
	u32 key;
	...
	if (!ret) {
		sunxi_debug(NULL, "get npu sid success, key is 0x%x, start update clk\n", key);
		sun55iw6_clk_update_npu(reg, key);
	}

	/*
	 * 1. Enable pll_xxx clk gate, power on default value is 1
	 * 2. Set pll_xxx auto gate to auto
	 */
	set_reg(reg + SUN55IW6_PLL_PERI0_EN_REG, 0x8fff0000, 32, 0);
	set_reg(reg + SUN55IW6_PLL_PERI1_EN_REG, 0x8ebf0000, 32, 0);
	set_reg(reg + SUN55IW6_PLL_VIDEO_EN_REG, 0x330000, 32, 0);

	ret = sunxi_ccu_probe(node, reg, &sun55iw6_ccu_desc);
	if (ret)
		return ret;

	sunxi_ccu_sleep_init(reg, sun55iw6_ccu_clks,
			ARRAY_SIZE(sun55iw6_ccu_clks),
			sun55iw6_distbus_restore_clks,
			ARRAY_SIZE(sun55iw6_distbus_restore_clks));
...
static void __init of_sun55iw6_ccu_init(struct device_node *node)
{
	sun55iw6_ccu_really_probe(node);
}

CLK_OF_DECLARE(sun55iw6_ccu_init, "allwinner,sun55iw6-ccu", of_sun55iw6_ccu_init);
#else
