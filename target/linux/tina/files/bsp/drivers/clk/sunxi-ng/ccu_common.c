...
static void ccu_restore_with_value(void __iomem *base,
			const struct ccu_reg_dump *all_rd,
			unsigned int all_num_regs,
			const struct ccu_reg_dump *rd,
			unsigned int num_regs)
{
	const struct ccu_reg_dump *restore_all_rd = all_rd;
	unsigned int restore_all_num_regs = all_num_regs;

	for (; num_regs > 0; --num_regs, ++rd) {
		for (; restore_all_num_regs > 0; --restore_all_num_regs, ++restore_all_rd) {
			if (rd->offset == restore_all_rd->offset) {
				writel(rd->value | restore_all_rd->value, base + rd->offset);
				break;
			}
		}
		restore_all_rd = all_rd;
		restore_all_num_regs = all_num_regs;
	}
}
...
static void ccu_resume(void)
{
	...
	list_for_each_entry(reg_cache, &ccu_reg_cache_list, node) {
		ccu_restore(reg_cache->reg_base, reg_cache->rdump,
				reg_cache->rd_num);
		ccu_restore_with_value(reg_cache->reg_base, reg_cache->rdump,
				reg_cache->rd_num, reg_cache->rsuspend,
