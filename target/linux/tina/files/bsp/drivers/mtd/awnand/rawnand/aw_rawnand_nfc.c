...
static void sunxi_nand_dma_cb_rx(void *data)
{
	awrawnand_dbg("dma rx callback\n");
}

static void sunxi_nand_dma_cb_tx(void *data)
{
...
{
	int ret = 0;
	host->dma_tx = dma_request_chan(host->dev, "tx");
	...
	host->dma_rx = dma_request_chan(host->dev, "rx");
	...
	awrawnand_info("request dma channel %s(tx) and %s(rx) success\n",
...
static int sunxi_nand_release_dma(struct aw_nand_host *host)
{
	if (!IS_ERR_OR_NULL(host->dma_tx))
		dma_release_channel(host->dma_tx);
	if (!IS_ERR_OR_NULL(host->dma_rx))
...
static int nand_dma_recv_start(struct aw_nand_host *host, void *pbuf, uint byte_cnt)
{
	u32 reg_val;
	...
	host->src_addr = host->base_addr_phy + NFC_REG_IO_DATA;
	host->dst_addr = dma_map_single(host->dev, pbuf, byte_cnt, DMA_BIDIRECTIONAL);
	if (dma_mapping_error(host->dev, host->dst_addr)) {
			dma_unmap_single(host->dev, host->dst_addr, byte_cnt, DMA_BIDIRECTIONAL);
			awrawnand_err("dma mapping err\n");
			return -EINVAL;
	}

	dma_conf.direction = DMA_DEV_TO_MEM;
	dma_conf.src_addr = host->src_addr;
	//dma_conf.dst_addr = host->dst_addr;

	if (byte_cnt % DMA_SLAVE_BUSWIDTH_4_BYTES == 0) {
		dma_conf.src_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
		dma_conf.dst_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
	} else if (byte_cnt % DMA_SLAVE_BUSWIDTH_2_BYTES == 0) {
		dma_conf.src_addr_width = DMA_SLAVE_BUSWIDTH_2_BYTES;
		dma_conf.dst_addr_width = DMA_SLAVE_BUSWIDTH_2_BYTES;
	} else {
	...
	dmaengine_slave_config(host->dma_rx, &dma_conf);

	dma_desc = dmaengine_prep_slave_single(host->dma_rx, host->dst_addr,
			byte_cnt, DMA_DEV_TO_MEM, DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
	if (!dma_desc) {
		awrawnand_err("dma rx prepare failed\n");
		return -EINVAL;
	}

	dma_desc->callback = sunxi_nand_dma_cb_rx;
	dma_desc->callback_param = (void *)host;
...
static int nand_dma_send_start(struct aw_nand_host *host, void *pbuf, uint byte_cnt)
{
	u32 reg_val;
	struct nfc_reg *nfc = &host->nfc_reg;
	struct dma_slave_config dma_conf = {0};
	...
	host->dst_addr = host->base_addr_phy + NFC_REG_IO_DATA;
	host->src_addr = dma_map_single(host->dev, pbuf, byte_cnt, DMA_TO_DEVICE);
	if (dma_mapping_error(host->dev, host->src_addr)) {
		dma_unmap_single(host->dev, host->src_addr, byte_cnt, DMA_TO_DEVICE);
			awrawnand_err("dma mapping err\n");
			return -EINVAL;
	}
	...
	dmaengine_slave_config(host->dma_tx, &dma_conf);

	dma_desc = dmaengine_prep_slave_single(host->dma_tx, host->src_addr,
			byte_cnt, DMA_MEM_TO_DEV, DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
	if (!dma_desc) {
		awrawnand_err("dma tx dmaengine_prep_slave_sg desc failed\n");
		return -EINVAL;
	}

	dma_desc->callback = sunxi_nand_dma_cb_tx;
	dma_desc->callback_param = (void *)host;
	dmaengine_submit(dma_desc);
...
static int aw_host_nfc_dma_config_start(struct aw_nand_host *host, uint8_t rw, void *addr, unsigned int len)
{
	...
	if (host->dma_type == MBUS_DMA) {
		...
		if (rw == FLASH_READ) {
			nand_dma_recv_start(host, addr, len);
		} else {
			nand_dma_send_start(host, addr, len);
		}
	}
...
dma_int_end:
	if (host->dma_type == MBUS_DMA) {
		...
		if (rw == FLASH_READ)
			dma_unmap_single(host->dev, host->dst_addr, len, dir);
		else
			dma_unmap_single(host->dev, host->src_addr, len, dir);
	}
...
static inline int aw_host_resource_init(struct aw_nand_host *host)
{
	...
	host->mem_res = platform_get_resource(host->pdev, IORESOURCE_MEM, 0);
	if (!host->mem_res) {
		awrawnand_err("failed to get mem resource\n");
		ret = -EINVAL;
		goto out;
	}

	host->base = devm_ioremap_resource(host->dev, host->mem_res);
	if (host->base == NULL) {
		awrawnand_err("fail to get nfc base reg\n");
		goto out;
	}
	host->base_addr_phy = host->mem_res->start;

	host->mdclk_val = 10;
	host->mcclk_val = 20;
#if IS_ENABLED(CONFIG_AW_MTD_RAWNAND_USE_GENERAL_DMA)
	host->dma_type = GENERIC_DMA;
#elif IS_ENABLED(CONFIG_AW_MTD_RAWNAND_USE_MBUS_DMA)
