...
#define SUNXI_RAWNAND_MODULE_VERSION	"1.0.4"
#define VERSION "v1.33 2024-06-27 17:15"
	...
	do {
		...
		if ((page_in_chip == buffer->simu_oobno) &&
			...
			if (ret == ECC_LIMIT) {
				...
				print_hex_dump(KERN_DEBUG, "", DUMP_PREFIX_OFFSET, 32, 1, pagebuf, buffer->simu_page_len, 1);
				awrawnand_err("spare len@%d\n", buffer->simu_oob_len);
				print_hex_dump(KERN_DEBUG, "", DUMP_PREFIX_OFFSET, 32, 1, oobbuf, buffer->simu_oob_len, 1);
				buffer->simu_pageno = INVALID_CACHE;
				buffer->simu_oobno = INVALID_CACHE;
				buffer->chipno = INVALID_CACHE;
...
#if IS_ENABLED(CONFIG_SUNXI_BOOT_PARAM)
#if IS_ENABLED(CONFIG_SIMULATE_MULTIPLANE)
	mtdparts[index++].size = 2 * phy_blk_bytes;
#else
	mtdparts[index++].size = phy_blk_bytes;
#endif /* CONFIG_SIMULATE_MULTIPLANE */
