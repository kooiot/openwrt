...
static int sunxi_rtc_gpr_len;
static void __iomem *sunxi_rtc_gpr_base;
...
{
	sunxi_info(dev, "%04d-%02d-%02d %02d:%02d:%02d\n",
...
static int sunxi_rtc_gettime(struct device *dev, struct rtc_time *tm)
{
	...
	if ((val & LOSC_STATUS) == 0)
		sunxi_warn(dev, "Warning: Using internal RC 16M clock source. Time may be inaccurate!\n");
	...
	case SUNXI_RTC_TYPE_B:
		...
		break;
	default:
		sunxi_err(dev, "Invalid chip->data->type = %d\n", chip->data->type);
		return -EINVAL;
	}

	err = rtc_valid_tm(tm);
	if (err) {
		sunxi_err(dev, "sunxi_rtc_gettime(): Invalid rtc_time: %ptR\n", tm);
		return err;
	}

	sunxi_debug(dev, "%s(): %ptR\n", __func__, tm);
	return err;
}
...
static int sunxi_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	...
	err = rtc_valid_tm(tm);
	if (err) {
		sunxi_err(dev, "sunxi_rtc_settime(): Invalid rtc_time: %ptR\n", tm);
		return err;
	}
	...
	if (rtc_valid_tm(tm)
		...
		sunxi_err(dev, "Invalid year %d. Should be in range %d ~ %d\n",
	...
	if (err) {
		sunxi_err(dev, "sunxi_rtc_settime(): wait_bits_cleared() timeout (1)\n");
		return err;
	}
	rtc_reg_write(chip, RTC_HHMMSS_REG, hw_hhmmss);
	err = wait_bits_cleared(chip, LOSC_CTRL_REG, RTC_HHMMSS_ACCESS(chip->data->hhmmss_access_bit), 50);
	if (err) {
		sunxi_err(dev, "sunxi_rtc_settime(): wait_bits_cleared() timeout (2)\n");
		return err;
	}
	...
	if (err) {
		sunxi_err(dev, "sunxi_rtc_settime(): wait_bits_cleared() timeout (3)\n");
		return err;
	}
	rtc_reg_write(chip, RTC_DAY_REG, hw_day);
	err = wait_bits_cleared(chip, LOSC_CTRL_REG, RTC_DAY_ACCESS(chip->data->day_access_bit), 50);
	if (err) {
		sunxi_err(dev, "sunxi_rtc_settime(): wait_bits_cleared() timeout (4)\n");
		return err;
	}
...
static void errata__fix_alarm_day_reg_default_value(struct device *dev)
{
	...
	if (rtc_reg_read(chip, ALARM0_DAY_REG) == 0) {
		sunxi_info(dev, "%s(): ALARM0_DAY_REG=0, set it to 1\n", __func__);
...
static int sunxi_rtc_getalarm(struct device *dev, struct rtc_wkalrm *wkalrm)
{
	...
	case SUNXI_RTC_TYPE_B:
		...
		if (err) {
			sunxi_err(dev, "Error getting time in getalarm\n");
			return err;
		}
	...
	err = rtc_valid_tm(tm);
	if (err)
		sunxi_err(dev, "sunxi_rtc_getalarm(): Invalid rtc_time: %ptR\n", tm);
...
static int sunxi_rtc_setalarm(struct device *dev, struct rtc_wkalrm *wkalrm)
{
	...
	err = rtc_valid_tm(tm);
	if (err) {
		sunxi_err(dev, "sunxi_rtc_setalarm(): Invalid rtc_time: %ptR\n", tm);
		return err;
	}
	...
	case SUNXI_RTC_TYPE_B:
		...
		if (err) {
			sunxi_err(dev, "sunxi_rtc_setalarm(): Failed to get time\n");
			return err;
		}
		sec_diff = rtc_tm_sub(tm, &tm_now);
		if (sec_diff <= 0) {
			sunxi_err(dev, "sunxi_rtc_setalarm(): Cannot set alarm in the past\n");
			return -EINVAL;
		} else if (sec_diff > TYPE_B_HW_SECONDS_MAX) {
			sunxi_err(dev, "sunxi_rtc_setalarm(): Time must be in the range\n");
			return -EINVAL;
		}
...
static irqreturn_t sunxi_rtc_alarm0_isr(int irq, void *id)
{
	...
	if (val & ALARM0_IRQ_PEND) {
		...
		sunxi_debug(chip->dev, "%s(): IRQ_HANDLED\n", __func__);
		return IRQ_HANDLED;
	}
...
void alarm0_ctrl(struct sunxi_rtc_dev *chip, bool en)
{
	...
	if (en) {
		...
		sunxi_debug(chip->dev, "%s(): enabled\n", __func__);
	} else {
		/* disable alarm0 */
		val = rtc_reg_read(chip, ALARM0_ENABLE_REG);
		val &= ~ALARM0_ENABLE;
		...
		sunxi_debug(chip->dev, "%s(): disabled\n", __func__);
	}
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= ARRAY_SIZE(str)) {
		sunxi_err(dev, "parameter is too long\n");
		return -EINVAL;
	}

	ret = kstrtou32(buf, 16, &val);
	if (ret) {
		sunxi_err(dev, "invalid para!\n");
		return -EINVAL;
	}
...
static int rtc_registers_access_check(struct sunxi_rtc_dev *chip)
{
	...
	void __iomem *addr = chip->gpr_base + chip->data->gpr_offset + 0 * 0x04;  /* The first General Purpose Register */
	u32 val_backup;
	...
	val_r = readl(addr);
	if (val_r != val_w) {
		sunxi_err(dev, "%s(): FAILED: Expect 0x%x but got 0x%x\n", __func__, val_w, val_r);
...
static int sunxi_rtc_poweroff_alarm(struct sunxi_rtc_dev *chip)
{
	...
	if (err) {
		sunxi_err(chip->dev, "%s(): get alarm failed\n", __func__);
		return err;
	}
	err = sunxi_rtc_gettime(chip->dev, &now);
	if (err) {
		sunxi_err(chip->dev, "%s(): get time failed\n", __func__);
		return err;
	}
	...
	sunxi_debug(chip->dev, "%s(): alarm0_en=%d, alarm0_irq_pending=%d, alarm0_expired=%d, alarm=%ptR, now=%ptR\n",
...
enum {
	...
	WRITE_SUNXI_RTC_BOOT_REASON_HOT_REBOOT,
	WRITE_SUNXI_RTC_BOOT_REASON_PANIC_REBOOT,
	WRITE_SUNXI_RTC_BOOT_REASON_MAX,
};
...
static void reboot_deal(void *data)
{
	...
	if (!data) {
		sunxi_info(NULL, "%s(): empty arg\n", __func__);
		return;
	}

	flag = str2flag(str);
	if (flag == 0) {
		sunxi_info(NULL, "%s(): unkown arg '%s'\n", __func__, str);
		return;
	}

	sunxi_info(NULL, "%s(): store flag '%s' (0x%x) in RTC General-Purpose-Register\n", __func__, str, flag);
...
{
	if (((value & 0xffff) >= WRITE_SUNXI_RTC_BOOT_REASON_MAX) ||
		IS_ERR_OR_NULL(boot_reasonbase)) {
		sunxi_err(NULL, "invalid reason or reasonbase NULL\n");
		return -1;
	}
	...
	if (value != readl(boot_reasonbase))
		sunxi_err(NULL, "set bootreason flag fail\n");
...
static int sunxi_rtc_bootreason_get(void)
{
	...
	if (IS_ERR_OR_NULL(boot_reasonbase)) {
		sunxi_err(NULL, "reasonbase NULL\n");
		return -1;
	}
...
static int sunxi_rtc_bootreason_init(void)
{
	...
	if ((bootreason & 0xffff) == SUNXI_RTC_BOOT_REASON_NONE) {
		/* none --> cold */
		sunxi_info(NULL, " Saving SoC boot-reason: COLD-BOOT\n");
		sunxi_rtc_bootreason_set(SUNXI_RTC_BOOT_REASON_COLD_BOOT);
	} else if ((bootreason & 0xffff) == WRITE_SUNXI_RTC_BOOT_REASON_HOT_REBOOT) {
		/* hot */
		sunxi_info(NULL, " Saving SoC boot-reason: HOT-BOOT\n");
		sunxi_rtc_bootreason_set(SUNXI_RTC_BOOT_REASON_HOT_REBOOT);
	} else if ((bootreason & 0xffff) == WRITE_SUNXI_RTC_BOOT_REASON_PANIC_REBOOT) {
		/* panic */
		sunxi_info(NULL, " Saving SoC boot-reason: PANIC-BOOT\n");
		sunxi_rtc_bootreason_set(SUNXI_RTC_BOOT_REASON_PANIC_REBOOT);
	} else {
		/* others, remain */
		sunxi_info(NULL, " Saving SoC boot-reason: OTHERS\n");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	flag = str2flag(str);
	if (flag == 0) {
		sunxi_err(dev, "unkown arg '%s'\n", str);
		return size;
	}
	sunxi_debug(dev, "store flag '%s' (0x%x) in RTC General-Purpose-Register\n", str, flag);
	writel(flag, gpr_cur_addr);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= 30) {
		sunxi_err(NULL, "parameters too long\n");
		return size;
	}
	...
	if (ret >= SUNXI_RTC_BOOT_REASON_MAX)
		sunxi_warn(NULL, "unknown reason %s", buf);
	else
		sunxi_info(NULL, "store rtc reason 0x%x\n", rtc_bootreason);
...
static int sunxi_rtc_reboot_flag_setup(struct sunxi_rtc_dev *chip)
{
	...
	if (err) {
		sunxi_err(dev, "Fail to read dts property '%s'\n", name);
		return err;
	}
	sunxi_debug(dev, "Read dts property '%s' = 0x%x\n", name, value);
	if (value >= chip->data->gpr_len) {
		sunxi_err(dev, "dts property '%s' is out of range!\n", name);
		return -EINVAL;
	}
	...
	if (err) {
		sunxi_err(dev, "Fail to read dts property 'gpr_bootcount_pos'\n");
	...
	if (err) {
		sunxi_err(dev, "Fail to read dts property 'gpr_fake_poweroff_pos'\n");
	...
	if (err) {
		sunxi_err(dev, "register reboot notifier error %d\n", err);
		return err;
	}

	err = atomic_notifier_chain_register(&panic_notifier_list, &panic_notifier);
	if (err) {
		sunxi_err(dev, "register panic notifier error %d\n", err);
		goto fail;
	}
	...
	if (err) {
		sunxi_err(dev, "device_create_file() failed\n");
...
{
	if (index >= sunxi_rtc_gpr_len || !sunxi_rtc_gpr_base) {
		sunxi_err(NULL, "get gpr value failed\n");
		return -1;
	}
	*val = readl(sunxi_rtc_gpr_base + index * 0x4);
	return 0;
}
EXPORT_SYMBOL(sunxi_rtc_get_gpr_val);

int sunxi_rtc_set_gpr_val(u8 index, u32 val)
{
	if (index >= sunxi_rtc_gpr_len || !sunxi_rtc_gpr_base) {
		sunxi_err(NULL, "set gpr value failed\n");
		return -1;
	}
	writel(val, sunxi_rtc_gpr_base + index * 0x4);
	return 0;
}
EXPORT_SYMBOL(sunxi_rtc_set_gpr_val);
...
static int sunxi_rtc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(chip->reset)) {
		sunxi_err(dev, "reset_control_get() failed\n");
		...
		*/
		...
		if (err) {
			sunxi_err(dev, "reset_control_deassert() failed\n");
			goto err1;
		}
	}
	...
	if (IS_ERR(chip->clk_bus)) {
		sunxi_err(dev, "Fail to get clock 'r-ahb-rtc'\n");
		err = PTR_ERR(chip->clk_bus);
		goto err2;
	}
	err = clk_prepare_enable(chip->clk_bus);
	if (err) {
		sunxi_err(dev, "Cannot enable clock 'r-ahb-rtc'\n");
		goto err2;
	}
	...
	if (IS_ERR(chip->clk)) {
		sunxi_warn(dev, "Fail to get clock 'rtc-1k'\n");
		/* Some SoCs have no such clock, like sun8iw11. Let's continue */
	} else {
		err = clk_prepare_enable(chip->clk);
		if (err) {
			sunxi_err(dev, "Cannot enable clock 'rtc-1k'\n");
			goto err3;
		}
	}
	...
	if (IS_ERR(chip->clk_spi))
		sunxi_warn(dev, "Fail to get clock 'rtc-spi'\n");
		/* Some SoCs have no such clock. Let's continue */
	else {
		err = clk_prepare_enable(chip->clk_spi);
		if (err) {
			sunxi_err(dev, "Cannot enable clock 'rtc-spi'\n");
			goto err4;
		}
	}
	...
	if (chip->data->gpr_base_address) {
		...
		if (IS_ERR_OR_NULL(chip->gpr_base)) {
			sunxi_err(dev, "Fail to map gpr base\n");
			err = -ENOMEM;
			goto err5;
		}
	} else {
	...
	sunxi_rtc_gpr_base = chip->gpr_base + chip->data->gpr_offset;

	sunxi_rtc_gpr_len = chip->data->gpr_len;
	...
	if (err) {
		sunxi_err(dev, "sunxi_rtc_reboot_flag_setup() failed\n");
		goto err5;
	}
	...
	err = devm_request_irq(dev, chip->irq, sunxi_rtc_alarm0_isr, 0,
				dev_name(dev), chip);
	if (err) {
		sunxi_err(dev, "Could not request IRQ\n");
		goto err6;
	}
	...
	if (IS_ERR(chip->rtc))  {
		sunxi_err(dev, "Unable to allocate RTC device\n");
...
#endif
	if (err) {
		sunxi_err(dev, "Unable to register RTC device\n");
		goto err6;
	}
...
out:
	sunxi_info(dev, "sunxi rtc probed\n");
	return 0;
...
static int __init sunxi_rtc_init(void)
{
	...
	if (err)
		sunxi_err(NULL, "register sunxi rtc failed\n");
