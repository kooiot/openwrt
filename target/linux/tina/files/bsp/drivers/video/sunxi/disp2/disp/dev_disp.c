				       ...
				       const char *buf, size_t count)
{
	...
	struct disp_capture_init_info cptr_init_info;
	...
	cptr_init_info.port = DISP_CAPTURE_AFTER_DEP;

	ret = mgr->cptr->start(mgr->cptr, &cptr_init_info);
	if (ret) {
...
static ssize_t disp_dump_regs_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return dump_regs(buf);
}

static ssize_t disp_dump_regs_force_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num;
	struct dump_regs_user_setting setting;

	get_de_user_setting(&setting);
	num = sprintf(buf, "%d\n", setting.force_dump);
	return num;
}

static ssize_t disp_dump_regs_force_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	int err;
	...
	set_dr_force_dump(val);

	return count;
}

static ssize_t disp_dump_regs_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num;
	struct dump_regs_user_setting setting;
	...
	get_de_user_setting(&setting);
	num = sprintf(buf, "%s\n", module_name[setting.type]);
	return num;
}

static ssize_t disp_dump_regs_type_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	char name[32] = {"\0"};
	int size = count > 32 ? 32 : count;

	memcpy(name, buf, size);
	name[size - 1] = '\0'; /* set end '\n' to '\0' */
	DUMP_REGS_PRINT("actual input str: %s\n", name);

	set_dr_module_type(name);

	return count;
}

static ssize_t disp_dump_regs_index_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num;
	struct dump_regs_user_setting setting;
	...
	get_de_user_setting(&setting);
	num = sprintf(buf, "%d %d %d\n", setting.index.disp, setting.index.chn, setting.index.id);
	return num;
}

static ssize_t disp_dump_regs_index_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	int err;
	...
	memcpy(string, buf, size - 1);
	string[size - 1] = '\0'; /* set end '\n' to '\0' */
	DUMP_REGS_PRINT("actual input str: %s\n", string);

	next = tosearch = string;
	if (tosearch == NULL)
		return count;
	next = strsep(&tosearch, " ");
	err = kstrtou32(next, 10, &index.disp);
	if (err) {
		DE_WARN("Invalid para\n");
		return err;
	}
	...
	if (tosearch == NULL)
		return count;
	next = strsep(&tosearch, " ");
	err = kstrtou32(next, 10, &index.chn);
	if (err) {
		DE_WARN("Invalid para\n");
		return err;
	}
	...
	if (tosearch == NULL)
		return count;
	next = strsep(&tosearch, " ");
	err = kstrtou32(next, 10, &index.id);
	if (err) {
		DE_WARN("Invalid para\n");
		return err;
	}
...
static ssize_t disp_dump_regs_debug_info_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", get_dr_debug_info());
}

static ssize_t disp_dump_regs_debug_info_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	int err;
...
static DEVICE_ATTR(dump_regs, 0660, disp_dump_regs_show, NULL);
static DEVICE_ATTR(dump_regs_force, 0660, disp_dump_regs_force_show, disp_dump_regs_force_store);
static DEVICE_ATTR(dump_regs_module, 0660, disp_dump_regs_type_show, disp_dump_regs_type_store);
static DEVICE_ATTR(dump_regs_index, 0660, disp_dump_regs_index_show, disp_dump_regs_index_store);
static DEVICE_ATTR(dump_regs_debug, 0660, disp_dump_regs_debug_info_show, disp_dump_regs_debug_info_store);
...
static struct attribute *disp_attributes[] = {
	...
	&dev_attr_dump_regs_force.attr,
	&dev_attr_dump_regs_module.attr,
	&dev_attr_dump_regs_index.attr,
	&dev_attr_dump_regs_debug.attr,
	NULL
};
...
static int __disp_dma_heap_alloc_coherent(struct disp_ion_mem *mem)
{
	...
	mem->vaddr = dma_alloc_coherent(g_disp_drv.dev, mem->size,  &dma_handle, GFP_KERNEL);
#endif
...
static inline void disp_init_bsp_init_para(struct disp_bsp_init_para *para, const struct disp_drv_info *drv_info)
{
	...
	for (i = 0; i < DISP_DEVICE_NUM; i++) {
		...
		para->rst_bus_tcon_bus[i] = drv_info->rst_bus_tcon_bus[i];
#endif
...
#if defined(HAVE_TCON_BUS_RESET_CLK)
		sprintf(id, "rst_bus_tcon_bus%d", i);
		disp_drv->rst_bus_tcon_bus[i] = devm_reset_control_get_shared(dev, id);
		if (IS_ERR(disp_drv->rst_bus_tcon_bus[i])) {
			disp_drv->rst_bus_tcon_bus[i] = NULL;
			DE_DEV_ERR(dev, "failed to get reset for %s\n", id);
			return -EINVAL;
		}
#endif
...
#if defined(HAVE_TCON_BUS_RESET_CLK)
		reset_control_put(disp_drv->rst_bus_tcon_bus[i]);
#endif
	}
...
handle_cmd:
	...
	case DISP_CAPTURE_START:
		{
			struct disp_capture_init_info info;
			...
			if (copy_from_user(&info, (void __user *)ubuffer[1],
			     sizeof(struct disp_capture_init_info))) {
				DE_WARN("copy_from_user fail\n");
				return -EFAULT;
			}

			if (cptr && cptr->start)
				ret = cptr->start(cptr, &info);
			break;
		}
