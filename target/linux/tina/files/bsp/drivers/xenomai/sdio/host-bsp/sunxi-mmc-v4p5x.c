				     ...
				     u32 pwr_save, u32 ignore_dat0)
{
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_CMDR, SDXC_START, 250);
...
int rtdm_sunxi_mmc_clk_set_rate_for_sdmmc_v4p5x(struct sunxi_mmc_host *host,
					   struct mmc_ios *ios)
{
	u32 mod_clk = 0;
	u32 src_clk = 0;
...
void rtdm_sunxi_mmc_thld_ctl_for_sdmmc_v4p5x(struct sunxi_mmc_host *host,
					struct mmc_ios *ios,
...
void rtdm_sunxi_mmc_save_spec_reg_v4p5x(struct sunxi_mmc_host *host)
{
...
void rtdm_sunxi_mmc_restore_spec_reg_v4p5x(struct sunxi_mmc_host *host)
{
	struct sunxi_mmc_spec_regs *spec_regs =
	    &((struct sunxi_mmc_ver_priv *)(host->version_priv_dat))->
	    bak_spec_regs;
...
bool rtdm_sunxi_mmc_opacmd23_v4p9(struct sunxi_mmc_host *host, bool set, u32 arg, u32 *rep)
{
	if (set) {
...
void rtdm_sunxi_mmc_set_ds_dl_raw(struct sunxi_mmc_host *host,
					 int sunxi_ds_dl)
{
	u32 rval;
...
void rtdm_sunxi_mmc_set_samp_dl_raw(struct sunxi_mmc_host *host,
					 int sunxi_samp_dl)
{
	u32 rval;
			...
			if (herrno) {
				...
				if (herrno & SDXC_INTERRUPT_CMD_ERROR_BIT) {
					if (host->sunxi_samp_dl_cnt >= SUNXI_MAX_RETRY_CNT_V4P5X)
						goto TO_HS;
				rtdm_sunxi_mmc_set_samp_dl_raw(host, (host->sunxi_samp_dl) % SUNXI_MAX_DELAY_POINT_V4P5X);
#ifdef SUNXI_RETRY_TEST
				rtdm_sunxi_mmc_set_samp_dl_raw(host, 52);
#endif
				...
				return 0;
					...
					rtdm_sunxi_mmc_set_ds_dl_raw(host, (host->sunxi_ds_dl) % SUNXI_MAX_DELAY_POINT_V4P5X);
#ifdef SUNXI_RETRY_TEST
					rtdm_sunxi_mmc_set_ds_dl_raw(host, 55);
#endif
...
TO_HS:
		...
		if (rcnt < SUNXI_MAX_RETRY_CNT_V4P5X) {
			rtdm_sunxi_mmc_set_samp_dl_raw(host, (host->sunxi_samp_dl) % 64);
			host->sunxi_samp_dl += SUNXI_MAX_RETRY_INTERVAL_V4P5X;
			return 0;
		}
...
void rtdm_sunxi_mmc_init_priv_v4p5x(struct sunxi_mmc_host *host,
			       struct platform_device *pdev, int phy_index)
{
	...
	host->sunxi_mmc_clk_set_rate = rtdm_sunxi_mmc_clk_set_rate_for_sdmmc_v4p5x;
	...
	host->sunxi_mmc_thld_ctl = rtdm_sunxi_mmc_thld_ctl_for_sdmmc_v4p5x;
	host->sunxi_mmc_save_spec_reg = rtdm_sunxi_mmc_save_spec_reg_v4p5x;
	host->sunxi_mmc_restore_spec_reg = rtdm_sunxi_mmc_restore_spec_reg_v4p5x;
...
EXPORT_SYMBOL_GPL(rtdm_sunxi_mmc_init_priv_v4p5x);

void rtdm_sunxi_mmc_init_priv_v4p6x(struct sunxi_mmc_host *host,
			       struct platform_device *pdev, int phy_index)
{
	struct sunxi_mmc_ver_priv *ver_priv =
	    devm_kzalloc(&pdev->dev, sizeof(struct sunxi_mmc_ver_priv),
	...
	if (mmc_readl(host, REG_SMCV) >= SMHC_VERSION_V4P9) {
		host->sunxi_mmc_opacmd23 = rtdm_sunxi_mmc_opacmd23_v4p9;
...
EXPORT_SYMBOL_GPL(rtdm_sunxi_mmc_init_priv_v4p6x);
