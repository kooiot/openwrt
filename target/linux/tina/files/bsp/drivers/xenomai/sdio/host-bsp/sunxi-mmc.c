...
static void rtdm_sunxi_mmc_handle_bottom_half(struct work_struct *work);
...
int rtdm_mmc_wbclr(struct sunxi_mmc_host *host, void *addr, u32 bmap, u32 tms)
{
 ...
 * set_ios() method.
 */
int rtdm_sunxi_mmc_regulator_set_ocr(struct mmc_host *mmc,
...
static inline int rtdm_sunxi_mmc_regulator_set_ocr(struct mmc_host *mmc,
			struct regulator *supply,
			unsigned short vdd_bit)
{
	return 0;
...
static int sunxi_mmc_reset_host(struct sunxi_mmc_host *host)
{
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_GCTRL, SDXC_HARDWARE_RESET, 250);
	if (ret)
...
static int sunxi_mmc_reset_dmaif(struct sunxi_mmc_host *host)
{
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_GCTRL, SDXC_DMA_RESET, 250);
	if (ret)
...
static int sunxi_mmc_reset_fifo(struct sunxi_mmc_host *host)
{
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_GCTRL, SDXC_FIFO_RESET, 250);
	if (ret)
...
static int sunxi_mmc_reset_dmactl(struct sunxi_mmc_host *host)
{
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_DMAC, SDXC_IDMAC_SOFT_RESET, 250);
	if (ret)
		SM_ERR(mmc_dev(host->mmc),
...
void rtdm_sunxi_mmc_set_a12a(struct sunxi_mmc_host *host)
{
	mmc_writel(host, REG_A12A, 0);
}
EXPORT_SYMBOL_GPL(rtdm_sunxi_mmc_set_a12a);
...
static void sunxi_mmc_start_ahb(struct sunxi_mmc_host *host,
				struct mmc_data *data)
{
	int i = 0, j = 0;
	u32 *buf = NULL;
	u32 fifo_status = (data->flags & MMC_DATA_WRITE) ? SDXC_FIFO_FULL : SDXC_FIFO_EMPTY;

	while (!(mmc_readl(host, REG_RINTR) & SDXC_COMMAND_DONE)) {
	}

	for (i = 0; i < data->sg_len; i++) {
		buf = host->sg_buf;
		for (j = 0; j < (data->sg[i].length >> 2); j++) {
			while (mmc_readl(host, REG_STAS) & fifo_status) {
			}

			if (data->flags & MMC_DATA_WRITE) {
				mmc_writel(host, REG_FIFO, buf[j]);
			} else {
				buf[j] = mmc_readl(host, REG_FIFO);
			}
...
static int rtdm_sunxi_mmc_irq(rtdm_irq_t *irqh)
{
	//struct sunxi_mmc_host *host = dev_id;
	struct sunxi_mmc_host *host = rtdm_irq_get_arg(irqh, struct sunxi_mmc_host);
	struct mmc_request *mrq;
	u32 msk_int, idma_int;
	bool finalize = false;
	bool sdio_int = false;
	int final_ret = 0;
	irqreturn_t ret = RTDM_IRQ_HANDLED;
...
/******************************************************/
	...
	if (final_ret == SUNXI_FINAL_BHALF)
		queue_work(system_wq, &host->sunxi_handle_bottom_work);

	return ret;
}

int rtdm_sunxi_check_r1_ready(struct sunxi_mmc_host *smc_host, unsigned ms)
{
...
int rtdm_sunxi_check_r1_ready_may_sleep(struct sunxi_mmc_host *smc_host)
{
	unsigned int cnt = 0;
	/*
...
{
	struct sunxi_mmc_host *host;
	host = container_of(work, struct sunxi_mmc_host, sunxi_handle_bottom_work);
	sunxi_mmc_handle_do_bottom_half(host);
}
...
static irqreturn_t sunxi_mmc_handle_do_bottom_half(void *dev_id)
{
	...
	if (mrq_busy) {
		 ...
		 *5 second
		 */
		rval = rtdm_sunxi_check_r1_ready_may_sleep(host);
		spin_lock_irqsave(&host->lock, iflags);
		if (rval) {
...
s32 rtdm_sunxi_mmc_update_clk(struct sunxi_mmc_host *host)
{
	u32 rval;
	s32 ret = 0;
	...
	ret = rtdm_mmc_wbclr(host, host->reg_base + SDXC_REG_CMDR, SDXC_START, 1000);
	if (ret)
		SM_ERR(mmc_dev(host->mmc),
...
static void sunxi_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (!IS_ERR(mmc->supply.vmmc)) {
			rval =
			    rtdm_sunxi_mmc_regulator_set_ocr(mmc, mmc->supply.vmmc,
						  ios->vdd);
			if (rval)
				return;
		}
	...
	case MMC_POWER_OFF:
		...
		if (!IS_ERR(mmc->supply.vmmc)) {
			rval = rtdm_sunxi_mmc_regulator_set_ocr(mmc, mmc->supply.vmmc, 0);
			if (rval)
				return;
		}
...
*/
	...
	host->mrq = mrq;
	host->wait_dma = (host->use_cpu) ? false : wait_dma;
	...
	if (unlikely(cmd->opcode == SD_SWITCH_VOLTAGE && (host->ctl_spec_cap & SUNXI_CMD11_TIMEOUT_DETECT))
		...
		queue_delayed_work(system_wq, \
				   ...
				   work_timeout);
	sunxi_mmc_exe_cmd(host, cmd, cmd_val, imask);
	if (data && host->use_cpu) {
		sunxi_mmc_start_ahb(host, data);
	}
/******************************************************/
	smp_wmb();
	spin_unlock_irqrestore(&host->lock, iflags);
...
static void sunxi_mmc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	//printk("********Function: %s, Line: %d - cmd->opcode = %d cmd->arg = 0x%x cmd->flags=0x%x\n", __func__, __LINE__, cmd->opcode, cmd->arg, cmd->flags);
...
static const struct of_device_id sunxi_mmc_of_match[] = {
	{.compatible = "allwinner,sun4i-a10-mmc-rtdm0test",},
	{ /* sentinel */ }
};
...
#if defined(MMC_FPGA) && defined(CONFIG_ARCH_SUN8IW10P1_RTDM)
				      ...
				      struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "allwinner,sun8iw10p1-sdmmc3")) {
		...
		host->sunxi_mmc_set_acmda = rtdm_sunxi_mmc_set_a12a;
		host->phy_index = 3;	/* 2; */
	}
#if defined(MMC_FPGA) && defined(CONFIG_ARCH_SUN8IW10P1_RTDM)
	enable_card3();
#endif	/* defined(MMC_FPGA) && defined(CONFIG_ARCH_SUN8IW10P1_RTDM) */
...
#if IS_ENABLED(CONFIG_AW_MMC_V4P1X_RTDM)
	if (of_device_is_compatible(np, "allwinner,sunxi-mmc-v4p1x-rtdm")) {
		int phy_index = 0;
		...
		rtdm_sunxi_mmc_init_priv_v4p1x(host, pdev, phy_index);
	}
#endif

#if IS_ENABLED(CONFIG_AW_MMC_V4P00X_RTDM)
	if ((of_device_is_compatible(np,
		...
		rtdm_sunxi_mmc_init_priv_v4p00x(host, pdev, phy_index);
	}
#endif

#if IS_ENABLED(CONFIG_AW_MMC_V4P10X_RTDM)
...
#ifdef CONFIG_AW_MMC_V4P5X_RTDM
	if (of_device_is_compatible(np, "allwinner,sunxi-mmc-v4p5x"))	{
		...
		rtdm_sunxi_mmc_init_priv_v4p5x(host, pdev, phy_index);
	}
	...
	if (of_device_is_compatible(np, "allwinner,sunxi-mmc-v4p6x"))	{
		...
		if (of_property_match_string(np, "device_type", "sdc0") == 0)
			...
			SM_ERR(&pdev->dev, "No sdmmc device,check dts\n");
		rtdm_sunxi_mmc_init_priv_v4p6x(host, pdev, phy_index);
	}
#endif

#ifdef CONFIG_AW_MMC_V5P3X_RTDM
	if (of_device_is_compatible(np, "allwinner,sunxi-mmc-v5p3x")) {
		...
		rtdm_sunxi_mmc_init_priv_v5p3x(host, pdev, phy_index);
	}
#endif
	...
	//ret = devm_request_threaded_irq(&pdev->dev, host->irq, sunxi_mmc_irq,
					...
					//host->name, host);
	ret = rtdm_irq_request(&host->irqh, host->irq, rtdm_sunxi_mmc_irq,
					0, host->name, host);
	if (ret) {
...
static int sunxi_mmc_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&host->sunxi_handle_bottom_work, rtdm_sunxi_mmc_handle_bottom_half);
	...
	/* Validate if the request was from software queue firstly. */
		...
		if (!ret) {
			...
			if (mmc_card_keep_power(mmc) || host->dat3_imask) {
				...
				if (!IS_ERR(mmc->supply.vmmc)) {
					ret =
					    rtdm_sunxi_mmc_regulator_set_ocr(mmc,
								  mmc->supply.
								  vmmc, 0);
					goto suspenderr;
				}
...
static int sunxi_mmc_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		if (mmc_card_keep_power(mmc) || host->dat3_imask) {
			...
			sunxi_mmc_regs_restore(host);
			host->ferror = rtdm_sunxi_mmc_update_clk(host);
			if (host->ferror) {
				ret = -1;
				goto resumerr;
			}
...
int32_t HAL_SDC_Update_Clk(struct mmc_host *host, uint32_t clk)
{
	...
	host->ios.vdd = 21;
	sunxi_mmc_set_ios(host, &(host->ios));
...
{
	sunxi_mmc_request(host, mrq);
	return 0;
}
void HAL_SDC_Enable_Sdio_Irq(struct mmc_host *host, int enable)
{
	return;
}
void HAL_SDC_set_timing(struct mmc_host *host, unsigned int timing)
{
	return;
}
...
int HAL_SDC_probe(struct platform_device *pdev)
{
	return sunxi_mmc_probe(pdev);
}

void rtdm_sunxi_shutdown_mmc(struct platform_device *pdev)
{
	struct mmc_host *mmc = platform_get_drvdata(pdev);
	struct sunxi_mmc_host *host = mmc_priv(mmc);
...
static struct platform_driver sunxi_mmc_driver = {
	...
	.shutdown = rtdm_sunxi_shutdown_mmc,
};
