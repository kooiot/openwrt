...
sunxi_mmc_show_perf(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Write performance at host driver Level:"
...
extern void rtdm_sunxi_mmc_set_ds_dl_raw(struct sunxi_mmc_host *host, int sunxi_ds_dl);
extern void rtdm_sunxi_mmc_set_samp_dl_raw(struct sunxi_mmc_host *host, int sunxi_samp_dl);
				...
				const char *buf, size_t count)
{
	...
	mmc_claim_host(mmc);
	rtdm_sunxi_mmc_set_samp_dl_raw(host, value);
	mmc_release_host(mmc);
				...
				const char *buf, size_t count)
{
	...
	mmc_claim_host(mmc);
	rtdm_sunxi_mmc_set_ds_dl_raw(host, value);
	mmc_release_host(mmc);
...
sunxi_mmc_show_filter_sector(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "filter speed %d\n", host->filter_sector);
}
...
sunxi_mmc_show_dbglevel(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Current debuglevel %d\n", host->debuglevel);
}
