...
#define SUNXI_SPI_DEV_NAME "sunxi-spi-ng-xenomai"
...
static inline struct spi_slave_sunxi *to_slave_sunxi(struct rtdm_spi_remote_slave *slave)
{
	return container_of(slave, struct spi_slave_sunxi, slave);
}

static inline struct sunxi_spi *to_master_sunxi(struct rtdm_spi_remote_slave *slave)
{
	return container_of(slave->master, struct sunxi_spi, master);
}
...
{
	struct rtdm_spi_master *master = &sspi->master;
	...
	if ((sspi->data->quirk_flag & NEW_SAMPLE_MODE) && (sspi->data->quirk_flag & NEW_SAMPLE_MODE_RST)) {
		u32 wait_cycle = sspi->pre_speed_hz ? ctlr->max_speed_hz / sspi->pre_speed_hz : 1;
...
static int sunxi_spi_set_clk(struct sunxi_spi *sspi, u32 clk)
{
	...
	if ((new_clk > ctlr->max_speed_hz) || (new_clk < ctlr->min_speed_hz)) {
...
static int sunxi_spi_resource_get(struct sunxi_spi *sspi)
{
	...
	sunxi_spi_resource_get_calibrate_xenomai(sspi);
...
static int sunxi_spi_bus_setup(struct sunxi_spi *sspi, struct rtdm_spi_remote_slave *slave)
{
	...
	if (config && config->speed_hz) {
		spi_speed_hz = config->speed_hz;

		spi_speed_hz = max(spi_speed_hz, ctlr->min_speed_hz);
		spi_speed_hz = min(spi_speed_hz, ctlr->max_speed_hz);
		if (spi_speed_hz != config->speed_hz)
	...
	if (sspi->pre_speed_hz != spi_speed_hz) {
		...
		case SUNXI_SPI_BUS_NOR:
		case SUNXI_SPI_BUS_NAND:
			sunxi_spi_set_delay_chain_xenomai(sspi, spi_speed_hz);
			break;
		case SUNXI_SPI_BUS_BIT:
			sunxi_spi_bit_sample_delay_xenomai(sspi, spi_speed_hz);
			break;
		default:
			break;
		}
...
static int sunxi_spi_xfer_setup(struct sunxi_spi *sspi, struct rtdm_spi_remote_slave *slave)
{
	struct rtdm_spi_config *config = &slave->config;
	int ret = 0;

	ret = sunxi_spi_bus_setup(sspi, slave);
	...
	case SUNXI_SPI_BUS_SLAVE:
	case SUNXI_SPI_BUS_CAMERA:
		sunxi_spi_config_tc(sspi, config->mode);
		break;
	case SUNXI_SPI_BUS_BIT:
		sunxi_spi_bit_config_tc_xenomai(sspi, config->mode);
		break;
	default:
...
static void sunxi_spi_set_cs(struct sunxi_spi *sspi, struct rtdm_spi_remote_slave *slave, bool status)
{
	struct rtdm_spi_config *config = &slave->config;
	...
	sunxi_spi_ss_polarity(sspi, !(config->mode & SPI_CS_HIGH));

	if (sspi->cs_mode == SUNXI_SPI_CS_SOFT)
		sunxi_spi_ss_level(sspi, status);
}

static int __maybe_unused sunxi_spi_slave_abort(struct spi_controller *ctlr)
{
...
static void __maybe_unused sunxi_spi_handle_err(struct spi_controller *ctlr, struct spi_message *message)
{
	struct sunxi_spi *sspi = spi_controller_get_devdata(ctlr);
...
static bool sunxi_spi_can_dma(struct sunxi_spi *sspi, struct spi_transfer *xfer)
{
...
static int sunxi_spi_mode_check(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	...
	switch (sspi->bus_mode) {
	case SUNXI_SPI_BUS_BIT:
		ret = sunxi_spi_mode_check_bit_xenomai(sspi, t);
		break;
	case SUNXI_SPI_BUS_DBI:
...
static int sunxi_spi_config_dma_rx(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	 ...
	 */
	if (spi_controller_is_slave(ctlr) && (sspi->data->quirk_flag & DMA_FORCE_FIXED)) {
	...
	dmaengine_slave_config(ctlr->dma_rx, &dma_conf);

	dma_desc = dmaengine_prep_slave_sg(ctlr->dma_rx, t->rx_sg.sgl,
...
static int sunxi_spi_config_dma_tx(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	...
	dmaengine_slave_config(ctlr->dma_tx, &dma_conf);

	dma_desc = dmaengine_prep_slave_sg(ctlr->dma_tx, t->tx_sg.sgl,
...
static int sunxi_spi_xfer_master(struct sunxi_spi *sspi, struct rtdm_spi_config *config, struct spi_transfer *t)
{
	struct rtdm_spi_master *master = &sspi->master;
	struct spi_controller *ctlr = master->controller;
	...
	if ((sspi->ready_gpio > 0) && (config->mode & SPI_READY)) {
	...
	timeout = rtdm_event_wait(&sspi->transfer_done);
	if (timeout) {
...
static int sunxi_spi_xfer_slave(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	struct rtdm_spi_master *master = &sspi->master;
	struct spi_controller *ctlr = master->controller;
	bool can_dma = sunxi_spi_can_dma(sspi, t);
...
static int sunxi_spi_xfer_dbi(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	struct rtdm_spi_master *master = &sspi->master;
	struct spi_controller *ctlr = master->controller;
	bool can_dma = sunxi_spi_can_dma(sspi, t);
...
static int sunxi_spi_transfer_one(struct sunxi_spi *sspi, struct rtdm_spi_remote_slave *slave, struct spi_transfer *t)
{
	struct rtdm_spi_master *master = &sspi->master;
	struct spi_controller *ctlr = master->controller;
	struct rtdm_spi_config *config = &slave->config;
	...
	if (config->mode & SPI_TX_QUAD) {
		t->tx_nbits = SPI_NBITS_QUAD;
	} else if (config->mode & SPI_TX_DUAL) {
		t->tx_nbits = SPI_NBITS_DUAL;
	}

	if (config->mode & SPI_RX_QUAD) {
		t->rx_nbits = SPI_NBITS_QUAD;
	} else if (config->mode & SPI_RX_DUAL) {
		t->rx_nbits = SPI_NBITS_DUAL;
	}

	ret = sunxi_spi_xfer_setup(sspi, slave);
	if (ret < 0) {
	...
	switch (sspi->bus_mode) {
	case SUNXI_SPI_BUS_BIT:
		sunxi_spi_bit_disable_irq_xenomai(sspi);
		sunxi_spi_bit_clr_irq_pending_xenomai(sspi);
		sunxi_spi_bit_enable_irq_xenomai(sspi);
		break;
	case SUNXI_SPI_BUS_DBI:
	...
	switch (sspi->bus_mode) {
	case SUNXI_SPI_BUS_DBI:
		ret = sunxi_spi_xfer_dbi(sspi, t);
		break;
	case SUNXI_SPI_BUS_BIT:
		ret = sunxi_spi_xfer_bit_xenomai(sspi, t);
		break;
	default:
		if (spi_controller_is_slave(ctlr))
			ret = sunxi_spi_xfer_slave(sspi, t);
		else {
			ret = sunxi_spi_xfer_master(sspi, config, t);
...
static void sunxi_spi_bus_handler(struct sunxi_spi *sspi)
{
	...
	if (compelte)
		rtdm_event_signal(&sspi->transfer_done);
}

static int sunxi_spi_handler(rtdm_irq_t *irqh)
{
	struct sunxi_spi *sspi;
	sspi = rtdm_irq_get_arg(irqh, struct sunxi_spi);
	...
	case SUNXI_SPI_BUS_DBI:
		...
		break;
	case SUNXI_SPI_BUS_BIT:
		sunxi_spi_bit_handler_xenomai(sspi);
		break;
	default:
...
static int sunxi_spi_clk_init(struct sunxi_spi *sspi, u32 clk)
{
	...
	if (clk < ctlr->min_speed_hz || clk > ctlr->max_speed_hz) {
		sunxi_warn(sspi->dev, "spi init clk %d not support, try %d as default\n", clk, ctlr->max_speed_hz);
		clk = ctlr->max_speed_hz;
	}
...
static int sunxi_spi_hw_init(struct sunxi_spi *sspi, u32 freq)
{
	...
	case SUNXI_SPI_BUS_NAND:
		...
		sunxi_spi_delay_chain_init_xenomai(sspi);
		break;
	case SUNXI_SPI_BUS_CAMERA:
	case SUNXI_SPI_BUS_SLAVE:
	...
	case SUNXI_SPI_BUS_DBI:
		...
		break;
	case SUNXI_SPI_BUS_BIT:
		sunxi_spi_bit_ss_owner_xenomai(sspi, sspi->cs_mode);
		break;
	default:
...
static int sunxi_spi_configure(struct rtdm_spi_remote_slave *slave)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);

	return sunxi_spi_xfer_setup(sspi, slave);
}

static void sunxi_chip_select(struct rtdm_spi_remote_slave *slave, bool active)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);

	sunxi_spi_set_cs(sspi, slave, active);
}

static ssize_t sunxi_spi_write(struct rtdm_spi_remote_slave *slave, const void *tx, size_t len)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);
	struct spi_transfer *t = sspi->xfer;
	t->len = len;
	t->tx_buf = tx;
	t->rx_buf = NULL;

	return sunxi_spi_transfer_one(sspi, slave, t) ? : len;
}

static ssize_t sunxi_spi_read(struct rtdm_spi_remote_slave *slave, void *rx, size_t len)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);
	struct spi_transfer *t = sspi->xfer;
	t->len = len;
...
static struct rtdm_spi_remote_slave *sunxi_spi_attach_slave(struct rtdm_spi_master *master, struct spi_device *spi)
{
	struct spi_slave_sunxi *sunxi_spi_slave;
	int ret;

	sunxi_spi_slave = kzalloc(sizeof(*sunxi_spi_slave), GFP_KERNEL);
	if (sunxi_spi_slave == NULL)
		return ERR_PTR(-ENOMEM);

	ret = rtdm_spi_add_remote_slave(&sunxi_spi_slave->slave, master, spi);
	if (ret) {
		sunxi_err(&spi->dev, "%s: failed to attach slave\n", __func__);
		kfree(sunxi_spi_slave);
		return ERR_PTR(ret);
	}

	sunxi_spi_slave->dev = &spi->dev;
	dma_set_coherent_mask(sunxi_spi_slave->dev, DMA_BIT_MASK(32));
	dma_set_max_seg_size(sunxi_spi_slave->dev, PAGE_SIZE);

	return &sunxi_spi_slave->slave;
}

static void sunxi_spi_detach_slave(struct rtdm_spi_remote_slave *slave)
{
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);

	rtdm_spi_remove_remote_slave(slave);
	kfree(sunxi_spi_slave);
}

static int sunxi_spi_open(struct rtdm_spi_remote_slave *slave)
{
	return 0;
}

static void sunxi_spi_close(struct rtdm_spi_remote_slave *slave) {}

static int set_iobufs(struct spi_slave_sunxi *spi_slave, size_t len)
{
	dma_addr_t dma;
	void *p;
	...
	if (len == spi_slave->io_len)
			return 0;
	if (spi_slave->io_len)
			return -EINVAL; /* I/O buffers may not be resized. */
	p = dma_alloc_coherent(spi_slave->dev, len, &dma, GFP_KERNEL);
	if (p == NULL)
			return -ENOMEM;
	spi_slave->io_dma = dma;
	spi_slave->io_virt = p;
	smp_mb();
	spi_slave->io_len = len;

	return 0;
}


static int sunxi_spi_set_iobufs(struct rtdm_spi_remote_slave *slave, struct rtdm_spi_iobufs *p)
{
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);
	int ret = 0;

	ret = set_iobufs(sunxi_spi_slave, p->io_len);
	if (ret)
		return ret;

	p->i_offset = 0;
	p->o_offset = sunxi_spi_slave->io_len / 2;
	p->map_len = sunxi_spi_slave->io_len;

	return 0;
}

static int sunxi_spi_mmap_iobufs(struct rtdm_spi_remote_slave *slave, struct vm_area_struct *vma)
{
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);
	vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);

	return rtdm_mmap_kmem(vma, sunxi_spi_slave->io_virt);
}

static void sunxi_spi_mmap_release(struct rtdm_spi_remote_slave *slave)
{
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);
	dma_free_coherent(sunxi_spi_slave->dev, sunxi_spi_slave->io_len, sunxi_spi_slave->io_virt, sunxi_spi_slave->io_dma);
	sunxi_spi_slave->io_len = 0;
}

static int sunxi_spi_transfer_iobufs(struct rtdm_spi_remote_slave *slave)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);
	struct spi_transfer *t = sspi->xfer;

	if (sunxi_spi_slave->io_len == 0)
		return -EINVAL;

	t->len = sunxi_spi_slave->io_len / 2;
	t->tx_buf = sunxi_spi_slave->io_virt + t->len;
	t->rx_buf = sunxi_spi_slave->io_virt;

	return sunxi_spi_transfer_one(sspi, slave, t);
}

static int sunxi_spi_transfer_iobufs_n(struct rtdm_spi_remote_slave *slave, int len)
{
	struct sunxi_spi *sspi = to_master_sunxi(slave);
	struct spi_slave_sunxi *sunxi_spi_slave = to_slave_sunxi(slave);
	struct spi_transfer *t = sspi->xfer;

	if ((sunxi_spi_slave->io_len == 0) || (len <= 0) || (len > (sunxi_spi_slave->io_len / 2)))
		return -EINVAL;

	t->len = len;
	t->tx_buf = sunxi_spi_slave->io_virt + (sunxi_spi_slave->io_len / 2);
	t->rx_buf = sunxi_spi_slave->io_virt;

	return sunxi_spi_transfer_one(sspi, slave, t);
}

static struct rtdm_spi_master_ops sunxi_spi_master_ops = {
	.open = sunxi_spi_open,
	.close = sunxi_spi_close,
	.configure = sunxi_spi_configure,
	.chip_select = sunxi_chip_select,
	.set_iobufs = sunxi_spi_set_iobufs,
	.mmap_iobufs = sunxi_spi_mmap_iobufs,
	.mmap_release = sunxi_spi_mmap_release,
	.transfer_iobufs = sunxi_spi_transfer_iobufs,
	.transfer_iobufs_n = sunxi_spi_transfer_iobufs_n,
	.write = sunxi_spi_write,
	.read = sunxi_spi_read,
	.attach_slave = sunxi_spi_attach_slave,
	.detach_slave = sunxi_spi_detach_slave,
};
...
static int sunxi_spi_probe(struct platform_device *pdev)
{
	...
	master = rtdm_spi_alloc_master(&pdev->dev, struct sunxi_spi, master);
	if (master == NULL)
		return -ENOMEM;
	master->subclass = 2;
	master->ops = &sunxi_spi_master_ops;

	sspi = container_of(master, struct sunxi_spi, master);
	sspi->pdev = pdev;
	sspi->dev = &pdev->dev;
	...
	rtdm_event_init(&sspi->transfer_done, 0);
	...
	if (IS_ERR_OR_NULL(ctlr)) {
		sunxi_err(sspi->dev, "failed alloc spi controller\n");
		ret = -ENOMEM;
		goto err0;
	}
	sspi->xfer = devm_kzalloc(sspi->dev, sizeof(*sspi->xfer), GFP_KERNEL);

	platform_set_drvdata(sspi->pdev, ctlr);
	spi_controller_set_devdata(ctlr, sspi);

	sspi->use_dma = false;

	/* spi controller configuration */
	ctlr->dev.of_node	= sspi->dev->of_node;
	ctlr->bus_num		= sspi->bus_num;
	ctlr->num_chipselect	= sspi->cs_num;
	ctlr->min_speed_hz	= SUNXI_SPI_MIN_FREQUENCY;
	ctlr->max_speed_hz	= sspi->bus_freq;
	ctlr->use_gpio_descriptors = true;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 5, 0))
	ctlr->max_native_cs	= SUNXI_SPI_CS_MAX;
#endif
	ctlr->max_dma_len		= PAGE_SIZE;
	ctlr->dma_alignment	= dma_get_cache_alignment();
	switch (sspi->bus_mode) {
	case SUNXI_SPI_BUS_SLAVE:
	case SUNXI_SPI_BUS_CAMERA:
		ctlr->bits_per_word_mask = SPI_BPW_MASK(8);
		ctlr->mode_bits = SPI_CPOL | SPI_CPHA | SPI_CS_HIGH | SPI_LSB_FIRST | sspi->data->slave_mode_extra;
		break;
	case SUNXI_SPI_BUS_MASTER:
	case SUNXI_SPI_BUS_NOR:
	case SUNXI_SPI_BUS_NAND:
	case SUNXI_SPI_BUS_DBI:
		ctlr->bits_per_word_mask = SPI_BPW_MASK(8);
		ctlr->mode_bits = SPI_CPOL | SPI_CPHA | SPI_CS_HIGH | SPI_LSB_FIRST | sspi->data->master_mode_extra;
	...
	case SUNXI_SPI_BUS_BIT:
		...
		ctlr->mode_bits = SPI_CS_HIGH | SPI_3WIRE;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
		ctlr->flags = SPI_CONTROLLER_HALF_DUPLEX | SPI_CONTROLLER_GPIO_SS;
#else
		ctlr->flags = SPI_CONTROLLER_HALF_DUPLEX | SPI_MASTER_GPIO_SS;
#endif
		break;
	default:
	...
	ret = rtdm_irq_request(&sspi->irqh, sspi->irq, sunxi_spi_handler, 0, dev_name(sspi->dev), sspi);
	if (ret) {
	...
	ret = rtdm_spi_add_master(&sspi->master);
	if (ret) {
	...
	sunxi_info(sspi->dev, "xenomai probe success (Version %s)\n", SUNXI_SPI_MODULE_VERSION);
...
static const struct of_device_id sunxi_spi_match[] = {
	/* sun8iw11 */
	{ .compatible = "allwinner,sunxi-spi-v0.90-xenomai", .data = &sunxi_spi_data_v0_90 },
	/* sun50iw9 */
	{ .compatible = "allwinner,sunxi-spi-v0.91-xenomai", .data = &sunxi_spi_data_v0_91 },
	/* sun8iw21/sun20iw5/sun20iw1 */
	{ .compatible = "allwinner,sunxi-spi-v1.1-xenomai", .data = &sunxi_spi_data_v1_1 },
	{ .compatible = "allwinner,sunxi-spi-v1.2-xenomai", .data = &sunxi_spi_data_v1_2 },
	/* sun55iw3/sun60iw1/sun60iw2/sun65iw1/sun252iw1 */
	{ .compatible = "allwinner,sunxi-spi-v1.3-xenomai", .data = &sunxi_spi_data_v1_3 },
	{ .compatible = "allwinner,sunxi-spi-v1.4-xenomai", .data = &sunxi_spi_data_v1_4 },
	/* sun55iw5/sun55iw6 */
	{ .compatible = "allwinner,sunxi-spi-v1.5-xenomai", .data = &sunxi_spi_data_v1_5 },
	{ .compatible = "allwinner,sunxi-spi-v1.6-xenomai", .data = &sunxi_spi_data_v1_6 },
	{},
};
...
MODULE_AUTHOR("liujingswc <jingyanliang@allwinnertech.com>");
