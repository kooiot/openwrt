...
#define SUNXI_FLASH_APP_MODULE_VERSION	"1.1.0"
	...
	while (len) {
		if (((uint32_t)to % erasesize) || (len < erasesize)) {
			if ((uint32_t)to % erasesize) {
				op_len = (uint32_t)to % erasesize;
				op_offset = erasesize - op_len;
			} else {
				op_len = len;
				op_offset = 0;
			}
			op_to = (to + erasesize -1) & ~(erasesize - 1);

			mtd_read(mtd, op_to, erasesize, &retlen, op_buf);
			...
			mtd_write(mtd, op_to, erasesize, &retlen, op_buf);
			...
			mtd_write(mtd, to, erasesize, &retlen, buf);
		...
		size_t len, const u_char *buf)
{
	...
	fp = filp_open(part_name, O_WRONLY, 0444);
	if (IS_ERR(fp)) {
		pr_err("open %s error\n", (char *)part_name);
		return -EINVAL;
	}

	ret = vfs_write(fp, buf, len, &to);
...
static int sunxi_rawpart_read(const void *part_name, loff_t from,
		size_t len, u_char *buf)
{
	if (!strncmp(part_name, "/dev/mtd", 8)) {
		char *endptr;
		int offset = strncmp(part_name, "mtdblock", 8) ? 3 : 8;
...
static int sunxi_rawpart_write(const void *part_name, loff_t to,
		size_t len, const u_char *buf)
{
	if (!strncmp(part_name, "/dev/mtd", 8)) {
		char *endptr;
