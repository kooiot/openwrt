				    ...
				    int histbinnum, int *curve, int curvebinnum)
{
	...
	/* avoiding stack overflow warn(1024 byte) */
	histtemp = kmalloc_array(32, sizeof(int), GFP_KERNEL | __GFP_ZERO);
	curveb = kmalloc_array(32, sizeof(int), GFP_KERNEL | __GFP_ZERO);
	curvew = kmalloc_array(32, sizeof(int), GFP_KERNEL | __GFP_ZERO);
	curveout = kmalloc_array(32, sizeof(int), GFP_KERNEL | __GFP_ZERO);
	curvenormal = kmalloc_array(32, sizeof(int), GFP_KERNEL | __GFP_ZERO);
	if (!histtemp || !curve || !curvew || !curveout || !curvenormal) {
		DRM_ERROR("dlc param alloc err\n");
		goto free;
	}
...
static int cal_yavg_from_hist(int *histtemp, int histbinnum)
{
	...
	/* just avoid warning about division by zero, histbinnum cannot be 0 */
	if (histbinnum <= 0)
		return yavg;
