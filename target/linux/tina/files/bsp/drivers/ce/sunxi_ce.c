...
static int sunxi_ss_hw_init(sunxi_ce_cdev_t *sss)
{
	...
	if (!IS_ERR_OR_NULL(sss->pclk))
		SS_DBG("SS ce_clk%luMHz, pclk %luMHz\n", clk_get_rate(sss->ce_clk) / 1000000, clk_get_rate(sss->pclk) / 1000000);
...
{
	int err = 0;
	sunxi_ce_cdev_t *ce_dev = NULL;

	ce_dev = devm_kzalloc(&pdev->dev, sizeof(*ce_dev), GFP_KERNEL);
	if (!ce_dev) {
...
#ifdef TASK_DMA_POOL
	ce_dev->task_pool = dma_pool_create("task_pool", &pdev->dev,
	...
	ce_dev->pdevice = &pdev->dev;
	snprintf(ce_dev->dev_name, sizeof(ce_dev->dev_name), SUNXI_SS_DEV_NAME);
	...
	SS_DBG("SS is inited, base 0x%px, irq %d!\n", ce_dev->base_addr, ce_dev->irq);
	return 0;

err3:
	sunxi_ss_hw_exit(ce_dev);
err2:
	sunxi_ss_res_release(ce_dev);
err1:
	platform_set_drvdata(pdev, NULL);
#ifdef TASK_DMA_POOL
	if (ce_dev->task_pool)
		dma_pool_destroy(ce_dev->task_pool);
#endif
err0:
	devm_kfree(&pdev->dev, ce_dev);
	return err;
}
