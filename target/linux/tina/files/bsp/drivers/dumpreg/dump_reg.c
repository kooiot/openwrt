...
static ssize_t __dump_regs_ex(struct dump_addr *dump_addr, char *buf, ssize_t buf_size)
{
	...
	if ((index < 0) || (index != __addr_valid(dump_addr->uaddr_end)) ||
	    (buf == NULL)) {
		sunxi_err(NULL, "%s(): Invalid para: index=%d, start=0x%lx, end=0x%lx, buf=0x%p\n",
	...
	for (uaddr = (dump_addr->uaddr_start & ~0x0F); uaddr <= dump_addr->uaddr_end;
		...
		if (cnt >= buf_size) {
			sunxi_warn(NULL, "Range too large, strings buffer overflow\n");
			cnt = buf_size;
			goto out;
		}
	...
	sunxi_debug(NULL, "%s(): start=0x%lx, end=0x%lx, return=%zd\n", __func__,
...
next:
	...
	if (!ret) {
		...
		sunxi_warn(NULL, "String syntax errors: \"%s\"\n", ptr);
...
#define WR_DATA_FMT PRINT_ADDR_FMT"  0x%08x  %s"
	...
	if (!pgroup) {
		sunxi_err(NULL, "%s,%d err, pgroup is NULL!\n", __func__, __LINE__);
		goto end;
	}
			     ...
			     size_t size)
{
	...
	do {
		...
		if (!__parse_write_str(ptr, &addr, &val)) {
			...
			sunxi_err(NULL, "%s: Failed to parse string: %s\n", __func__,
			       ptr);
...
#endif
{
	...
	sunxi_info(NULL, "standby_dump_ctrl change to 0x%08x\n", standby_dump_ctrl);
...
#endif
{
	...
	sunxi_debug(NULL, "%s,%d, start_reg:" PRINT_ADDR_FMT ", end_reg:" PRINT_ADDR_FMT
...
static int sunxi_dump_reg_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(dump_class)) {
		sunxi_err(NULL, "%s:%u class_create() failed\n", __func__, __LINE__);
	...
	for (i = 0; i < ARRAY_SIZE(dump_class_attrs); i++) {
		...
		if (err) {
			sunxi_err(NULL, "%s:%u class_create_file() failed. err=%d\n", __func__, __LINE__, err);
			while (i--) {
	...
	if (standby_dump_buff != NULL)
		standby_dump_buff = NULL;

	standby_dump_buff = devm_kmalloc(dev, PAGE_SIZE, GFP_KERNEL);
	if (!standby_dump_buff)
		sunxi_err(dev, "malloc memory failed.\n");

	err = of_property_read_u32(np, "ddr-range", &value);
	if (!err)
		dump_table[1].addr_end = (SUNXI_PLAT_PHYS_START + ((long long)value * SZ_1G) - 1);
	register_syscore_ops(&standby_dump_syscore);

	return 0;
error:
	sunxi_err(dev, "sunxi_dump_reg probe error\n");
	return -1;
}
