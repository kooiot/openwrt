...
#define SUNXI_RPROC_TRACE_VERSION "1.0.6"
...
static inline int ringbuffer_get(void *rb, int rb_size, void *_data, size_t max_size)
{
	...
	if (!writer->size) {
		if (reader->flags & WRITER_NOT_INIT) {
			read = -EBUSY;
			goto out;
		}
		if (max_size > 1) {
...
ssize_t sunxi_rproc_trace_log_read_to_user(void *from, int buf_len, char __user *userbuf,
...
void *parser_addr_map_on_linux_kernel(const aw_trace_event_parser_t *parser, uint64_t da, size_t len)
{
	...
	va = rproc_da_to_va(rproc, da, len, NULL);
#else
	va = rproc_da_to_va(rproc, da, len);
#endif

	return va;
}

typedef struct user_print_desc_on_kernel {
	/* we need to copy buf data from kernel space to user space after print operation */
	uint8_t *kernel_buf;
	uint32_t kernel_buf_len;

	uint8_t *user_buf;
	uint32_t user_buf_len;
	uint32_t user_buf_write_index;

	aw_trace_event_parser_t *parser;
} user_print_desc_on_kernel_t;

int user_print_func_on_kernel(void *priv, const char *fmt, ...)
{
    va_list args;
    int r, ret;
	uint8_t *kernel_buf, *current_user_buf;
	uint32_t kernel_buf_len, user_buf_write_len, user_buf_len, user_buf_write_index;

	user_print_desc_on_kernel_t *pr_desc;
	aw_trace_event_parser_t *parser;
	struct rproc *rproc;
	struct device *dev;

	pr_desc = priv;
	parser = pr_desc->parser;
	rproc = parser->priv;
	dev = rproc->dev.parent;

	kernel_buf = pr_desc->kernel_buf;
	kernel_buf_len = pr_desc->kernel_buf_len;

    va_start(args, fmt);
    r = vsnprintf(kernel_buf, kernel_buf_len, fmt, args);
    va_end(args);

	user_buf_write_len = r;
	if (r >= kernel_buf_len) {
		dev_err(dev, "event print buffer in kernel space is too small! real: %u, need: %d",
			kernel_buf_len, r);
		user_buf_write_len = kernel_buf_len - 1;
	}

	user_buf_len = pr_desc->user_buf_len;
	user_buf_write_index = pr_desc->user_buf_write_index;

	if ((user_buf_write_index + user_buf_write_len) >= user_buf_len) {
		ret = BUF_SPACE_NOT_ENOUGH;
		return ret;
	}

	current_user_buf = &pr_desc->user_buf[user_buf_write_index];
	ret = copy_to_user(current_user_buf, kernel_buf, user_buf_write_len);
	if (ret)
		dev_err(dev, "remoteproc trace event copy_to_user failed");

	pr_desc->user_buf_write_index += user_buf_write_len;

    return r;
}

static ssize_t rproc_aw_trace_event_read(struct file *filp, char __user *userbuf,
				size_t count, loff_t *ppos)
{
	int ret;
	struct rproc_debug_trace *data = filp->private_data;
	struct rproc_mem_entry *trace_mem = &data->trace_mem;
	struct device *dev = data->rproc->dev.parent;
	struct sunxi_rproc *aw_rproc = data->rproc->priv;
	aw_trace_event_parser_t *parser = &aw_rproc->trace_event_parser;

	reader_info_t *r_info, reader_info;
	user_print_desc_on_kernel_t pr_desc;

	if (!parser->trace_event.events_buf) {
		dev_info(dev, "init the trace event object on this platform\n");
		ret = aw_trace_event_obj_init(&parser->trace_event, parser, (aw_trace_event_t *)trace_mem->va);
		if (ret) {
			dev_err(dev, "init aw trace event object failed, ret: %d\n", ret);
			return 0;
		}
	}

	/* copy to temporary variable */
	reader_info = parser->reader_info;
	r_info = &reader_info;

	dev_dbg(dev, "read_pos: %u, last_event: pos=%u, timestamp=%llu",
		r_info->read_pos, r_info->last_event_pos,
		r_info->last_event_timestamp);

	r_info->trace_event_obj = &parser->trace_event;
	r_info->is_read_all = 0;
	r_info->is_read_to_buf = 1;
	r_info->is_panic_when_read = 0;

	memset(&pr_desc, 0, sizeof(pr_desc));
	pr_desc.kernel_buf = parser->kernel_buf;
	pr_desc.kernel_buf_len = sizeof(parser->kernel_buf);
	pr_desc.user_buf = userbuf;
	pr_desc.user_buf_len = count;
	pr_desc.parser = parser;

	r_info->user_pr_func = user_print_func_on_kernel;
	r_info->user_pr_priv_data = &pr_desc;

	ret = aw_trace_event_read(parser, r_info);
	if (ret)
		dev_err(dev, "aw_trace_event_dump failed, ret: %d, event_data_len: %u\n",
			ret, pr_desc.user_buf_write_index);

	/* update the original data */
	parser->reader_info = reader_info;

	return pr_desc.user_buf_write_index;
}
#else
static ssize_t rproc_aw_trace_event_read(struct file *filp, char __user *userbuf,
				size_t count, loff_t *ppos)
{
	const char *info_str = "aw trace event parser is not available!\n";
	return simple_read_from_buffer(userbuf, count, ppos, info_str, strlen(info_str));
}
#endif

static const struct file_operations g_aw_trace_event_file_ops = {
	.read = rproc_aw_trace_event_read,
	.open = simple_open,
	.llseek	= generic_file_llseek,
};

static ssize_t rproc_aw_trace_log_read(struct file *filp, char __user *userbuf,
				size_t count, loff_t *ppos)
{
	struct rproc_debug_trace *data = filp->private_data;
	...
	return sunxi_rproc_trace_log_read_to_user(va, trace->len, userbuf, count, ppos);
}

static const struct file_operations g_aw_trace_log_file_ops = {
	.read = rproc_aw_trace_log_read,
	.open = simple_open,
	.llseek	= generic_file_llseek,
};
				       ...
				       struct rproc_debug_trace *trace)
{
	...
	if (!strncmp(trace_mem->name, "event", 32)) {
		fops = &g_aw_trace_event_file_ops;
	} else {
		fops = &g_aw_trace_log_file_ops;
	}
