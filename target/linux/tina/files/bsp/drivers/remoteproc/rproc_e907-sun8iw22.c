...
static struct rproc_common_res common_res[] = {
	...
	{ .res_name = RV_CORE_GATE_CLK_NAME, .type = RPROC_COMMON_CLK_RES },
	{ .res_name = "rv-ts-gate", .type = RPROC_COMMON_CLK_RES },
	{ .res_name = "rv-cfg-gate", .type = RPROC_COMMON_CLK_RES },
	{ .res_name = "axi-div", .type = RPROC_COMMON_CLK_RES },
	{ .res_name = "axi-monitor", .type = RPROC_COMMON_CLK_RES },
	/* rst */
	{ .res_name = "rv-cfg-rst", .type = RPROC_COMMON_RST_RES },
	{ .res_name = "core-rst", .type = RPROC_COMMON_RST_RES },
	{ .res_name = "rv-sys-rst", .type = RPROC_COMMON_RST_RES },
	{ .res_name = "rst-axi-monitor", .type = RPROC_COMMON_RST_RES },
	/* reg */
	{ .res_name = RV_IO_RES_NAME, .type = RPROC_COMMON_REG_RES },
	{ .res_name = RV_AXI_MONITOR_RES_NAME, .type = RPROC_COMMON_REG_RES }
};
...
static struct rproc_clk_opration common_start_seq[]  = {
	...
	{ .name = "core-gate",   .option = RPROC_COMMON_OP(SETPARENT), .parent = "input" },
	{ .name = "core-gate",   .option = RPROC_COMMON_OP(SETRATE), .prop_rate = RV_DTS_PROPERTY_CORE_CLK_FREQ },
	{ .name = "axi-div",   .option = RPROC_COMMON_OP(SETRATE), .prop_rate = RV_DTS_PROPERTY_AXI_CLK_FREQ },
	//{ .name = "rst-axi-monitor", .option = RPROC_COMMON_OP(ASSERT) },
	//{ .name = "axi_rst_delay", .option = RPROC_COMMON_OP(USLEEP) , .val = 500},
#if IS_ENABLED(CONFIG_AW_REMOTEPROC_E907_AXI_MONITOR)
	{ .name = "rst-axi-monitor", .option = RPROC_COMMON_OP(DEASSERT) },
	{ .name = "axi-monitor", .option = RPROC_COMMON_OP(ENABLE) },
	{ .name = RV_AXI_MONITOR_RES_NAME, .option = RPROC_COMMON_OP(WRITEREG), .offset = AXI_MONITOR_TAKEN_OVER_REG },
	{ .name = RV_AXI_MONITOR_RES_NAME, .option = RPROC_COMMON_OP(WRITEREG), .offset = AXI_MONITOR_TAKEN_TIMEOUT_REG },
#endif
	...
	{ .name = "rv-ts-gate", .option = RPROC_COMMON_OP(ENABLE) },
...
static struct rproc_clk_opration common_stop_seq[]  = {
	...
	{ .name = "rv-ts-gate", .option = RPROC_COMMON_OP(DISABLE) },
	{ .name = "core-rst",  .option = RPROC_COMMON_OP(ASSERT) },
	{ .name = "rv-sys-rst", .option = RPROC_COMMON_OP(ASSERT) },
	{ .name = "rv-cfg-rst", .option = RPROC_COMMON_OP(ASSERT) },
#if IS_ENABLED(CONFIG_AW_REMOTEPROC_E907_AXI_MONITOR)
	{ .name = "rst-axi-monitor", .option = RPROC_COMMON_OP(ASSERT) },
	{ .name = "axi_rst_delay", .option = RPROC_COMMON_OP(USLEEP), .val = 1000},
	{ .name = "rst-axi-monitor", .option = RPROC_COMMON_OP(DEASSERT) },
#endif
};
