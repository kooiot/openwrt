...
#define SUNXI_RPROC_VERSION "2.4.4"
...
static int sunxi_rproc_request_mbox(struct rproc *rproc)
{
	...
	sched_set_fifo_low(chip->mb.rx_daemon);
#else
...
static int sunxi_rproc_stop(struct rproc *rproc)
{
	...
	if (rproc->state == RPROC_CRASHED) {
		...
		dev_info(rproc_priv->dev, "begin dump trace mem data before restart remoteproc!\n");
		...
		list_for_each_entry_safe(trace, tmp, &rproc->traces, node) {
			...
			if (va) {
				...
				if (!strncmp(trace_mem->name, "event", 32)) {
					dev_info(rproc_priv->dev, "found aw trace event mem! va: %px\n", va);
#ifdef CONFIG_AW_RPROC_TRACE_EVENT_PARSER
					print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 16, 1, va, sizeof(aw_trace_event_t), 1);
					aw_trace_event_info_dump(va);
					aw_trace_event_dump_all(&chip->trace_event_parser, va, 0, _printk);
#else
					dev_info(rproc_priv->dev, "aw trace event parser is not available!\n");
#endif
				} else {
					sunxi_rproc_trace_dump(va, trace_mem->len);
				}
			} else {
				dev_err(rproc_priv->dev, "failed to dump trace mem data, trace mem: 0x%08x, len: %zu\n", trace_mem->da, trace_mem->len);
			}
		}
	}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	trace_mem_va = rproc_da_to_va(rproc, rsc->da, rsc->len, NULL);
#else
	trace_mem_va = rproc_da_to_va(rproc, rsc->da, rsc->len);
#endif

	if (!trace_mem_va) {
		dev_err(dev, "get trace mem va failed, trace mem: 0x%08x, len: %u\n",
			rsc->da, rsc->len);
		goto exit_with_free_obj;
	}

	trace->trace_mem.va = trace_mem_va;
	trace->trace_mem.da = rsc->da;
	trace->trace_mem.len = rsc->len;

	memcpy(trace->trace_mem.name, rsc->name, sizeof(trace->trace_mem.name));
...
#else
	dev_warn(dev, "can't add trace mem '%s', da: 0x%08x, len: %u, please enable aw trace mem cfg!",
			name, rsc->da, rsc->len);
	return 0;
#endif
