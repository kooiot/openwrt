...
static void send_sgi_to_amp_os(u64 cluster_id, u16 tlist, unsigned int irq)
{
	u64 val;
...
static void dead_work_func(struct work_struct *work)
{
	...
	struct sunxi_rproc_arm64_cfg *cfg;

	cfg = container_of(dead_work, struct sunxi_rproc_arm64_cfg, dead_work);

	arm_smccc_smc(PSCI_CPU_STATE, cfg->cpu_id, 0, 0, 0, 0, 0, 0, &res);

	/* res.a0 == 0， mean cpu is off  */
	if (!res.a0) {
...
static int devm_sunxi_rproc_arm64_resource_get(struct sunxi_rproc_priv *rproc_priv, struct platform_device *pdev)
{
	...
	/* TODO : delete */
	INIT_DELAYED_WORK(&arm64_cfg->dead_work, dead_work_func);
	init_completion(&arm64_cfg->complete_dead);
...
static void devm_sunxi_rproc_arm64_resource_put(struct sunxi_rproc_priv *rproc_priv, struct platform_device *pdev)
{
	struct sunxi_rproc_arm64_cfg *arm64_cfg = rproc_priv->rproc_cfg;

	cancel_delayed_work_sync(&arm64_cfg->dead_work);
	reinit_completion(&arm64_cfg->complete_dead);
...
static int sunxi_rproc_arm64_cpu_hotplug(int cpu_id, bool online)
{
	int ret, reg_len;
	int i = 0, logic_cpu = 0;
...
static int sunxi_rproc_arm64_start(struct sunxi_rproc_priv *rproc_priv)
{
	...
	sunxi_rproc_arm64_cpu_hotplug(cfg->cpu_id, false);	/* cpu offline in kernel */
	...
	dev_info(dev, "%s,%d, PSCI_CPU_ON %x, start addr 0x%x\n", __func__, __LINE__,
...
static int sunxi_rproc_arm64_stop(struct sunxi_rproc_priv *rproc_priv)
{
	...
	send_sgi_to_amp_os(cfg->cpu_id, 1, CPU_OFF_IRQ);
	...
	sunxi_rproc_arm64_cpu_hotplug(cfg->cpu_id, true);	/* cpu online in kernel */
...
static struct sunxi_rproc_ops sunxi_rproc_arm64_ops = {
	...
	.resource_put = devm_sunxi_rproc_arm64_resource_put,
...
static int __init sunxi_rproc_arm64_rtos_boot_init(void)
{
	int ret;

	ret = sunxi_rproc_priv_ops_register("arm64_rtos", &sunxi_rproc_arm64_ops, NULL);
	if (ret) {
...
subsys_initcall(sunxi_rproc_arm64_rtos_boot_init);

static void __exit sunxi_rproc_arm64_rtos_boot_exit(void)
{
	int ret;

	ret = sunxi_rproc_priv_ops_unregister("arm64_rtos");
	if (ret)
		pr_err("arm unregister ops failed\n");
}
module_exit(sunxi_rproc_arm64_rtos_boot_exit)

/* arm64_boot_init must run before sunxi_rproc probe */
static int __init sunxi_rproc_arm64_baremetal_boot_init(void)
{
	int ret;

	ret = sunxi_rproc_priv_ops_register("arm64_baremetal", &sunxi_rproc_arm64_ops, NULL);
	if (ret) {
		pr_err("arm register ops failed\n");
		return ret;
	}
...
subsys_initcall(sunxi_rproc_arm64_baremetal_boot_init);

static void __exit sunxi_rproc_arm64_baremetal_boot_exit(void)
{
	int ret;

	ret = sunxi_rproc_priv_ops_unregister("arm64_baremetal");
	if (ret)
		pr_err("arm unregister ops failed\n");
}
module_exit(sunxi_rproc_arm64_baremetal_boot_exit)
