...
static int sunxi_init_arch_info(struct device_node *child, struct arch_irq *arch)
{
	...
	irm_debug("map table from 0x%zx to 0x%px\n", (size_t)r.start, va);
...
static int sunxi_update_gpio_mask(struct arch_irq *arch)
{
	int i, j;
	uint32_t type, index, major, mask;
	uint32_t max_hwirq = 0, irq_info_table_len, gpio_irq_info_table_len;
	struct _table_head *irq_tab_head;
	struct _table_entry *irq_tab_entry;
	...
	for (i = 0; i < arch->entris; i++) {
		...
		for (j = 0; j < ARRAY_SIZE(share_irq_table); j++) {
			if (share_irq_table[j].major == major) {
				...
				irm_debug("\tGPIO%c -> %d\n", 'A' + index, arch->gpio_irq_map[index][1]);

				if (max_hwirq < share_irq_table[j].hwirq)
					max_hwirq = share_irq_table[j].hwirq;
	...
	irq_info_table_len = sizeof(*irq_tab_head) + (max_hwirq + 1) * sizeof(*irq_tab_entry);
	gpio_irq_info_table_len = CPU_BANK_NUM * sizeof(uint32_t);
	if ((irq_info_table_len + gpio_irq_info_table_len) > arch->len) {
		irm_err("The length(%u) of shared memory is not enough! "
			"irq_info_table_len: %u, gpio_irq_info_table_len: %u\n",
			arch->len, irq_info_table_len, gpio_irq_info_table_len);
		return -ENOMEM;
	}
...
static int __init sunxi_share_resource_init(void)
{
	...
	for_each_child_of_node(np, child) {
		if (sunxi_init_arch_info(child, &g_arch_array[cnt]))
			goto out;
		ret = sunxi_update_gpio_mask(&g_arch_array[cnt]);
		if (ret)
			goto out;

		sunxi_update_table(&g_arch_array[cnt]);
		g_arch_array[cnt].visable = true;
		cnt++;
	}
