...
static int check_os_online(struct sunxi_rproc_arm_cfg *cfg)
{
	...
	arm_smccc_smc(PSCI_CPU_STATE, cfg->cpu_id[0], 0, 0, 0, 0, 0, 0, &res);
...
{
	uint32_t val;
	val = sunxi_optee_call_amp(AMP_CPU_STATE, cfg->cpu_id[0], 0);
					...
					struct platform_device *pdev)
{
	...
	n = of_property_count_elems_of_size(amp_node, "cpu-id", sizeof(u32));
	if (n < 0) {
		dev_err(dev, "no cpu-id node \n");
		return -ENXIO;
	}

	ret = of_property_read_u32_array(amp_node, "cpu-id", arm_cfg->cpu_id, n);
	if (ret) {
		dev_err(dev, "no \"cpu-id\" node specified\n");
		return -ENXIO;
	}

	/* only n cpus, set 0 in cpu_id[n] */
	arm_cfg->cpu_id[n] = 0;
...
static int sunxi_rproc_arm_start(struct sunxi_rproc_priv *rproc_priv)
{
	...
	for (i = 0; i < AMP_CPUS; i++) {
		if (cfg->cpu_id[i] == 0)
			break;
		sunxi_rproc_arm_cpu_hotplug(cfg->cpu_id[i], false);	/* cpu offline in kernel */
	}
	...
	arm_smccc_smc(PSCI_CPU_ON, cfg->cpu_id[0], rproc_priv->pc_entry, 0, 0, 0, 0, 0, &res);

	dev_dbg(dev, "%s,%d, PSCI_CPU_ON %x, start addr 0x%x\n", __func__, __LINE__,
...
static int sunxi_rproc_arm_stop(struct sunxi_rproc_priv *rproc_priv)
{
	...
	dev_dbg(rproc_priv->dev, "amp cpu %x run ---- \n", cfg->cpu_id[0]);
	...
	for (i = 0; i < AMP_CPUS; i++) {
		if (cfg->cpu_id[i] == 0) /* ID = 0, is a invalid val */
			break;
		sunxi_rproc_arm_cpu_hotplug(cfg->cpu_id[i], true);	/* cpu online in kernel */
	}
