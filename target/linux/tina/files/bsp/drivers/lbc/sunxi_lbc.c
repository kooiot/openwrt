...
#include <linux/fs.h>
#include "sunxi_lbc.h"
#include "sunxi_lbc_common.h"

#define LBC_DEBUG

#define LBC_VERSION "1.0.5"
...
static void sunxi_lbc_dma_set_iodl(struct __lbc_idma_des *p, size_t iodl)
{
	p->desc0.reg.desc_15t0_iodl = iodl & 0xffff;
}
...
static int lbc_dma_buf_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	struct sunxi_lbc_t *lbc_dev = dmabuf->priv;
	void *vaddr;
	dma_addr_t dma_addr;
	printk("lbc: dma_buf mmap called for buffer\n");

	printk("*** dmabuf addr: %p\n", dmabuf);
	printk("*** dma_tx_dmabuf: %p\n", lbc_dev->dma_tx_buffer);
	printk("*** dma_rx_dmabuf: %p\n", lbc_dev->dma_rx_buffer);

	if (dmabuf == lbc_dev->dma_tx_dmabuf) {
		vaddr = lbc_dev->dma_tx_buffer;
		dma_addr = lbc_dev->dma_tx_buffer_phy;
		printk("mapping dma tx buffer\n");
	} else if (dmabuf == lbc_dev->dma_rx_dmabuf) {
		vaddr = lbc_dev->dma_rx_buffer;
		dma_addr = lbc_dev->dma_rx_buffer_phy;
		printk("mapping dma rx buffer\n");
	} else {
		printk("invalid dmabuf\n");
		return -EINVAL;
	}

	return dma_mmap_coherent(lbc_dev->dev, vma, vaddr, dma_addr, DMA_DATA_MAX);
}

static void lbc_dma_buf_release(struct dma_buf *dmabuf)
{
	struct sunxi_lbc_t *lbc_dev = dmabuf->priv;
	if (dmabuf == lbc_dev->dma_tx_dmabuf) {
		printk("dma tx buf release");
	} else if (dmabuf == lbc_dev->dma_rx_dmabuf) {
		printk("dma rx buf release");
	} else {
		printk("invalid dmabuf\n");
	}
}

static struct sg_table *lbc_dma_buf_map(struct dma_buf_attachment *attachment, enum dma_data_direction direction)
{
	struct sunxi_lbc_t *lbc_dev = attachment->dmabuf->priv;
	struct sg_table *sgt;
	int ret;
	dma_addr_t dma_addr;
	bool is_tx = false;

	if (attachment->dmabuf == lbc_dev->dma_tx_dmabuf) {
		dma_addr = lbc_dev->dma_tx_buffer_phy;
		is_tx = true;
		printk("lbc: mapping TX buffer for DMA, %s:%d\n", __func__, __LINE__);

		if (lbc_dev->tx_sgt)
			return lbc_dev->tx_sgt;
	} else if (attachment->dmabuf == lbc_dev->dma_rx_dmabuf) {
		dma_addr = lbc_dev->dma_rx_buffer_phy;
		is_tx = false;
		printk("lbc: mapping RX buffer for DMA, %s:%d\n", __func__, __LINE__);

		if (lbc_dev->rx_sgt)
			return lbc_dev->rx_sgt;
	} else {
		printk("lbc: unknown dma_buf in map, %s:%d\n", __func__, __LINE__);
	...
	sg_dma_len(sgt->sgl) = DMA_DATA_MAX;
	...
	printk("current map sgt: 0x%p, %s:%d\n", sgt, __func__, __LINE__);
	return sgt;
}

static void lbc_dma_buf_unmap(struct dma_buf_attachment *attachment,
							 struct sg_table *sgt,
							 enum dma_data_direction direction)
{
	printk("current unmap sgt: 0x%p, %s:%d\n", sgt, __func__, __LINE__);
	sg_free_table(sgt);
	kfree(sgt);
}

static const struct dma_buf_ops lbc_dma_buf_ops = {
	.map_dma_buf = lbc_dma_buf_map,
	.unmap_dma_buf = lbc_dma_buf_unmap,
	.mmap = lbc_dma_buf_mmap,
	.release = lbc_dma_buf_release,
};
				...
				size_t count, loff_t *ppos)
{
	...
	writel((lbc_dev->dma_tx_phy >> 2), (lbc_dev->reg_addr + LOCAL_BUSR_REG_DESC_SADDR));
	sunxi_lbc_dma_set_target_addr(lbc_dev->dma_tx, (lbc_dev->dma_target_addr + lbc_dev->dma_f_pos));
	sunxi_lbc_dma_set_iodl(lbc_dev->dma_tx, count);
#ifdef LBC_DEBUG
	sunxi_lbc_dump_dma_desc(lbc_dev->dma_tx, 1);
#endif
	dma_transfer_start(lbc_dev->reg_addr);
				...
				size_t count, loff_t *ppos)
{
	...
	writel((lbc_dev->dma_rx_phy >> 2), (lbc_dev->reg_addr + LOCAL_BUSR_REG_DESC_SADDR));
	sunxi_lbc_dma_set_target_addr(lbc_dev->dma_rx, (lbc_dev->dma_target_addr + lbc_dev->dma_f_pos));
	sunxi_lbc_dma_set_iodl(lbc_dev->dma_rx, count);
#ifdef LBC_DEBUG
	sunxi_lbc_dump_dma_desc(lbc_dev->dma_rx, 1);
#endif
	dma_transfer_start(lbc_dev->reg_addr);
...
static long sunxi_lbc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct dma_trans_params trans_params;
	memset(&trans_params, 0, sizeof(trans_params));

	switch (cmd) {
	case CMD_DMA_TX_START:
		if (copy_from_user((void *)&trans_params, (void __user *)arg, sizeof(trans_params))) {
			return -EFAULT;
		}

		lbc_dev->dma_tx->desc0.reg.desc_18t16_burst_r = trans_params.burst_type;
		lbc_dev->dma_tx->target_start_addr = trans_params.target_addr;
		lbc_dev->dma_tx->desc0.reg.desc_15t0_iodl = trans_params.data_length;
		cs = (trans_params.cs & 0xF) << 12;  // 确保只取低4位， 移动到12 - 15 bit
		mb();
...
#ifdef LBC_DEBUG
		printk("---------------------TX------------------------\n");
		printk("dma_trans_params tsa: 0x%x\n", trans_params.target_addr);
		printk("		  burst_type: 0x%x\n", trans_params.burst_type);
		printk("		burst_length: 0x%x\n", trans_params.burst_length);
		printk("				iodl: 0x%x\n", trans_params.data_length);
		sunxi_lbc_dump_dma_desc(lbc_dev->dma_tx, 1);
#endif
		// 根据用户层传输的CS值进行12 - 15 bit的置位
		reg_val = readl(lbc_dev->reg_addr + LOCAL_BUSR_REG_CSR);
		reg_val = (reg_val & ~(0xF << 12)) | cs;  // 先清零12-15位，再写入cs
		writel(reg_val, lbc_dev->reg_addr + LOCAL_BUSR_REG_CSR);
		printk(" Yika cs: %d, reg 0x%08x , value = 0x%08x\n", cs, LOCAL_BUSR_REG_CSR, readl(lbc_dev->reg_addr + LOCAL_BUSR_REG_CSR));
		dma_transfer_start(lbc_dev->reg_addr);
	...
	case CMD_DMA_RX_START:
		...
		lbc_dev->dma_rx->desc0.reg.desc_18t16_burst_r = trans_params.burst_type;
		lbc_dev->dma_rx->target_start_addr = trans_params.target_addr;
		lbc_dev->dma_rx->desc0.reg.desc_15t0_iodl = trans_params.data_length;
		mb();
...
#endif
		break;
	case CMD_EXPORT_DMA_TX_BUF:
		printk("lbc: export tx dma buf\n");
		exp_info.ops = &lbc_dma_buf_ops;
		exp_info.size = DMA_DATA_MAX;
		exp_info.flags = O_RDWR;
		exp_info.priv = lbc_dev;

		dmabuf = dma_buf_export(&exp_info);
		if (IS_ERR(dmabuf)) {
			printk("failed to export tx dma buf\n");
			return PTR_ERR(dmabuf);
		}

		printk("dmabuf addr: %p\n", dmabuf);
		lbc_dev->dma_tx_dmabuf = dmabuf;

		fd = dma_buf_fd(dmabuf, O_CLOEXEC);
		if (fd < 0) {
			printk("lbc: failed to get fd from TX dma-buf\n");
			dma_buf_put(dmabuf);
			return fd;
		}
		...
		printk("lbc: TX dma-buf exported with fd %d\n", fd);
		break;
case CMD_EXPORT_DMA_RX_BUF:
		printk("lbc: exporting RX dma-buf\n");
		...
		dmabuf = dma_buf_export(&exp_info);
		if (IS_ERR(dmabuf)) {
			printk("lbc: failed to export RX dma-buf\n");
			return PTR_ERR(dmabuf);
		}

		printk("dmabuf addr: %p\n", dmabuf);
		lbc_dev->dma_rx_dmabuf = dmabuf;

		fd = dma_buf_fd(dmabuf, O_CLOEXEC);
		if (fd < 0) {
			printk("lbc: failed to get fd from RX dma-buf\n");
			dma_buf_put(dmabuf);
			return fd;
		}
		...
		printk("lbc: RX dma-buf exported with fd %d\n", fd);
		break;
	default:
		break;
	}
