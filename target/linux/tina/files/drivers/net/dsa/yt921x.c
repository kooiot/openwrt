...
int yt921x_dsa_get_mac_eee(struct dsa_switch *ds, int port,
...
static int
yt921x_dsa_port_lag_leave(struct dsa_switch *ds, int port, struct net_device *lag)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct dsa_port *dp;
	u32 ctrl;
	...
	mutex_lock(&priv->reg_lock);
	res = yt921x_lag_set(priv, id - 1, ctrl);
	mutex_unlock(&priv->reg_lock);
...
static int
yt921x_dsa_port_lag_check(struct dsa_switch *ds, struct net_device *lag,
...
static int
yt921x_dsa_port_lag_join(struct dsa_switch *ds, int port, struct net_device *lag,
			 struct netdev_lag_upper_info *info)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	struct dsa_port *dp;
	...
	res = yt921x_dsa_port_lag_check(ds, lag, info);
	if (res)
		return res;
...
yt921x_vlan_filtering(struct yt921x_priv *priv, int port, bool vlan_filtering)
{
	...
	struct net_device *bdev = dp->bridge_dev;
	u16 pvid;
	u32 mask;
	u32 ctrl;
	int res;
				...
				struct net_device *br)
{
	...
	ports_mask = dsa_bridge_ports(ds, br);
	ports_mask |= priv->cpu_ports_mask;
...
static void
yt921x_phylink_mac_link_down(struct dsa_switch *ds, int port, unsigned int mode,
			     phy_interface_t interface)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
...
static void
yt921x_phylink_mac_link_up(struct dsa_switch *ds, int port, unsigned int mode,
...
static void
yt921x_phylink_mac_config(struct dsa_switch *ds, int port, unsigned int mode,
